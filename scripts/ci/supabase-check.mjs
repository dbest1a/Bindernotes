...
for (const item of migrations)
  if (baseline === "clean" || item.version <= checkpoint)
    await writeFile(path.join(migrationDirectory, item.name), item.sql);
...
function sql(input) {
  const result = spawnSync("psql", pgArgs, {
    input,
    env: pgEnv,
    encoding: "utf8",
...
function concurrentSql(input) {
  return new Promise((resolve) => {
    const child = spawn("psql", pgArgs, { env: pgEnv });
    let stdout = "";
    let stderr = "";
...
if (baseline !== "clean") {
  if (baseline === "production-observed") {
    ...
    for (const version of ["0020", "0021", "0022"]) {
      const item = migrations.find((migration) => migration.version === version);
      assert(item);
      sql(item.sql);
    }
  }
  ...
  for (const item of migrations)
    if (item.version > checkpoint) await writeFile(path.join(migrationDirectory, item.name), item.sql);
...
const generatedTypes = spawnSync(
  process.execPath,
  ["scripts/database/generate-types.mjs", "--ci-supabase", "--check"],
...
const fixture = JSON.parse(
  command(process.execPath, ["--import", "tsx", "scripts/database/catalog-fixture.ts"]),
);
...
for (const name of ["learner-a", "learner-b", "creator", "operator"]) {
  const email = `${name}@bindernotes.invalid`;
  ...
  if (name === "creator")
    sql(
      `insert into public.account_entitlements(user_id,plan,status) values('${data.user.id}','studio','active');`,
    );
}
await writeFile(
  path.join(directory, "runtime.json"),
  JSON.stringify({ apiUrl: apiUrl.origin, anonKey: status.ANON_KEY, users, run: randomUUID() }),
);
console.log(
  "PASS full Supabase schema, role/concurrency suites, catalogs and real disposable Auth accounts prepared",
);
