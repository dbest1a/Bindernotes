...
if (process.argv.includes("--ci-supabase")) {
  ...
  assert.match(explicitDatabase, /^bindernotes_test_[a-f0-9]{32}$/);
  database = explicitDatabase;
  const host = process.env.BINDERNOTES_TEST_DB_HOST ?? "127.0.0.1";
  assert(["127.0.0.1", "localhost", "::1"].includes(host), "Only disposable loopback catalogs");
  psql = path.join(process.env.BINDERNOTES_PG_BIN ?? "", process.platform === "win32" ? "psql.exe" : "psql");
  port = process.env.BINDERNOTES_TEST_DB_PORT ?? port;
  user = process.env.BINDERNOTES_TEST_DB_USER ?? user;
} else {
  ...
  psql = path.join(runtime, "pgsql/bin/psql.exe");
}
...
const result = spawnSync(
  ...
  { input: query, encoding: "utf8", windowsHide: true, maxBuffer: 16 * 1024 * 1024 },
);
...
function typeOf(type, category, element) {
  ...
  if (["int2", "int4", "int8", "float4", "float8", "numeric", "money", "oid"].includes(type)) return "number";
...
for (const table of schema.tables.filter((t) => t.kind === "r")) {
  ...
  for (const mode of ["Row", "Insert", "Update"]) {
    ...
    for (const column of table.columns) {
      const optional =
        mode === "Update" ||
        (mode === "Insert" && (column.nullable || column.default || column.identity || column.generated));
      const type =
        column.generated && mode !== "Row"
          ? "never"
          : typeOf(column.type, column.category, column.element) + (column.nullable ? " | null" : "");
      output += `${JSON.stringify(column.name)}${optional ? "?" : ""}: ${type};\n`;
    }
    output += "};\n";
  }
  output +=
    ...
    (table.relationships ?? [])
      .map(
        (rel) =>
          `{ foreignKeyName: ${JSON.stringify(rel.name)}; columns: ${JSON.stringify(rel.columns)}; isOneToOne: ${rel.oneToOne}; referencedRelation: ${JSON.stringify(rel.target)}; referencedColumns: ${JSON.stringify(rel.targetColumns)} }`,
      )
      .join(",") +
    "];\n};\n";
}
const views = schema.tables.filter((t) => t.kind !== "r");
output += "};\nViews: " + (views.length ? "{\n" : "Record<PropertyKey, never>;\n");
for (const view of views) {
  output +=
    `${JSON.stringify(view.name)}: { Row: {` +
    view.columns
      .map((c) => `${JSON.stringify(c.name)}: ${typeOf(c.type, c.category, c.element)} | null;`)
      .join("") +
    "}; Relationships: [] };\n";
}
output += (views.length ? "};\n" : "") + "Functions: {\n";
...
for (const fn of schema.functions ?? []) {
  const input = (fn.args ?? []).filter((a) => ["i", "b", "v"].includes(a.mode));
  const out = (fn.args ?? []).filter((a) => ["o", "b", "t"].includes(a.mode));
  const args = input.length
    ? "{" +
      input
        .map(
          (a, i) =>
            `${JSON.stringify(a.name)}${i >= input.length - fn.defaults ? "?" : ""}: ${typeOf(a.type, a.category, a.element)}${a.type === "json" || a.type === "jsonb" ? "" : " | null"};`,
        )
        .join("") +
      "}"
    : "Record<PropertyKey, never>";
  const returns = out.length
    ? "{" +
      out.map((a) => `${JSON.stringify(a.name)}: ${typeOf(a.type, a.category, a.element)};`).join("") +
      "}"
    : typeOf(fn.returnType, fn.returnCategory, fn.returnElement);
  const contract = `{ Args: ${args}; Returns: ${returns}${fn.set ? "[]" : ""} }`;
  grouped.set(fn.name, [...(grouped.get(fn.name) ?? []), contract]);
}
for (const [name, contracts] of grouped) output += `${JSON.stringify(name)}: ${contracts.join(" | ")};\n`;
output += "};\nEnums: {\n";
for (const e of schema.enums ?? [])
  output += `${JSON.stringify(e.name)}: ${e.values.map((v) => JSON.stringify(v)).join(" | ")};\n`;
...
if (process.argv.includes("--check"))
  assert.equal(
    ...
    output,
    "Generated Database types differ from the real local schema",
  );
else await writeFile(target, output, "utf8");
console.log(
  `${process.argv.includes("--check") ? "Verified" : "Generated"} ${schema.tables.filter((t) => t.kind === "r").length} tables and ${grouped.size} RPC names from actual local PostgreSQL catalog. No row data or secrets emitted.`,
);
