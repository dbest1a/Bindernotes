...
assert(
  process.env.BINDERNOTES_LOCAL_RUNTIME,
  "Set BINDERNOTES_LOCAL_RUNTIME to the outside-repository native runtime directory",
);
assert(
  !runtime.startsWith(projectRoot + path.sep),
  "Keep runtime data and credentials outside the repository",
);
...
function sql(statement, db = database) {
  const result = spawnSync(psql, [...base, "-d", db], {
    input: statement,
    encoding: "utf8",
    windowsHide: true,
...
const env = {
  ...
  DB_NAMESPACE: "auth",
  DATABASE_URL: `postgres://postgres@127.0.0.1:55439/${database}?sslmode=disable&search_path=auth,public,extensions`,
  API_EXTERNAL_URL: apiUrl,
  GOTRUE_API_HOST: "127.0.0.1",
  PORT: "55440",
  GOTRUE_DB_MAX_POOL_SIZE: "3",
  GOTRUE_DB_MAX_IDLE_POOL_SIZE: "1",
  GOTRUE_MAILER_AUTOCONFIRM: "true",
  GOTRUE_DISABLE_SIGNUP: "false",
  GOTRUE_EXTERNAL_EMAIL_ENABLED: "true",
  GOTRUE_EXTERNAL_PHONE_ENABLED: "false",
  GOTRUE_SITE_URL: "http://127.0.0.1:5173",
  GOTRUE_URI_ALLOW_LIST: "http://127.0.0.1:*,http://localhost:*",
...
async function requireFreePort(port) {
  ...
  assert(
    !listening,
    `Local port ${port} is already occupied; do not replace an existing stack or its credential manifest`,
  );
}
...
try {
  ...
  created = true;
  const fixture = (
    await readFile(path.join(projectRoot, "scripts/database/platform-fixture.sql"), "utf8")
  ).replace(/^create table auth\.(users|sessions)[^\n]*\n/gm, "");
  assert(!fixture.includes("create table auth.users"), "GoTrue must own the complete auth.users schema");
  sql(fixture);
  const migration = spawnSync(path.join(runtime, "gotrue.exe"), ["migrate"], {
    cwd: runtime,
    env,
    encoding: "utf8",
    windowsHide: true,
  ...
  assert.equal(
    migration.status,
    0,
    "Real GoTrue migration failed; inspect gotrue-migrate.log outside repository",
  );
  ...
  start(
    path.join(runtime, "postgrest/postgrest.exe"),
    [],
    {
      ...process.env,
      PATH: `${path.join(runtime, "pgsql/bin")}${path.delimiter}${process.env.PATH}`,
      PGRST_DB_URI: `postgres://bindernotes_local_authenticator@127.0.0.1:55439/${database}?sslmode=disable`,
      PGRST_DB_SCHEMAS: "public",
      PGRST_DB_ANON_ROLE: "anon",
      PGRST_JWT_SECRET: secret,
      PGRST_SERVER_HOST: "127.0.0.1",
      PGRST_SERVER_PORT: "55441",
      PGRST_DB_POOL: "5",
  ...
  start(
    process.execPath,
    [path.join(projectRoot, "scripts/database/local-api-proxy.mjs")],
    process.env,
    "local-api-proxy",
  );
  ...
  for (const name of ["learner-a", "learner-b", "creator", "operator"]) {
    const user = {
      name,
      email: `${name}@bindernotes.invalid`,
  ...
  const operator = users.find((user) => user.name === "operator");
  const creator = users.find((user) => user.name === "creator");
  ...
  sql(
    `begin; set local role service_role; select public.apply_catalog_seed('${JSON.stringify(JSON.parse(seed.stdout)).replaceAll("'", "''")}'::jsonb);commit;`,
  );
  const manifest = {
    database,
    apiUrl,
    ...
    limitations: [
      "Windows GoTrue startup patch removes Unix SO_REUSEPORT only",
      "No pg_cron worker/registration",
      "No Storage HTTP or Realtime service",
    ],
  };
  await writeFile(path.join(runtime, "local-api-stack.json"), JSON.stringify(manifest, null, 2), "utf8");
  console.log(
    `PASS: real local GoTrue + PostgREST started; four disposable users and catalogs created. API ${apiUrl}.`,
  );
  console.log(
    "Credentials were written outside the repository to local-api-stack.json; no credentials printed.",
  );
} catch (error) {
