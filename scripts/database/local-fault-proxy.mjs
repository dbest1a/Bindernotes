...
const savePaths = new Set([
  "/rest/v1/rpc/save_personal_content",
  "/rest/v1/rpc/save_whiteboard_snapshot",
  "/rest/v1/rpc/save_review_item",
  "/rest/v1/rpc/save_review_session",
]);
function mode() {
  try {
    const value = JSON.parse(fs.readFileSync(control, "utf8")).mode;
    return ["pass", "reject", "hold"].includes(value) ? value : "reject";
  } catch {
    return "reject";
  }
}
...
const server = http.createServer((request, response) => {
  ...
  if (isSave && mode() === "reject") {
    ...
    response.end(
      JSON.stringify({ message: "Local test connection interrupted. Retry when connectivity returns." }),
    );
  ...
  const upstream = http.request(
    {
      ...
      path: request.url,
      headers: { ...request.headers, host: "127.0.0.1:55442" },
    },
    (remote) => {
...
server.listen(55443, "127.0.0.1", () =>
  console.log("Disposable browser fault gateway on loopback55443; local control file only"),
);
