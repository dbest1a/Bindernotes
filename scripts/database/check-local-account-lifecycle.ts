...
try{
 ...
 const statusResponse=await fetch(`${manifest.apiUrl}/rest/v1/rpc/get_account_session_status`,{method:"POST",headers,body:"{}"});assert.equal(statusResponse.status,200);assert.equal(await statusResponse.json(),"revoked");
