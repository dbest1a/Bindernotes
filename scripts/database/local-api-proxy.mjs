...
http
  .createServer((request, response) => {
    ...
    if (!route) {
      ...
      response.end(
        '{"error":"This local stack only runs real Auth and PostgREST. Storage and Realtime services are not configured."}',
      );
      return;
    }
    const upstream = http.request(
      {
        ...
        headers: { ...request.headers, host: `127.0.0.1:${route.port}` },
      },
      (remote) => {
        for (const [name, value] of Object.entries(remote.headers))
          if (value !== undefined && !name.startsWith("access-control-")) response.setHeader(name, value);
    ...
    upstream.on("error", () => {
      ...
      response.end("Local service unavailable");
    });
    request.pipe(upstream);
  })
  .listen(55442, "127.0.0.1", () =>
    console.log("Local Auth/PostgREST gateway listening on loopback port55442"),
  );
