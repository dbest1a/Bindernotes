...
const psql = path.join(process.env.BINDERNOTES_PG_BIN ?? '', process.platform === 'win32' ? 'psql.exe' : 'psql');
...
assert(!upgradeFrom || ['0015','0025','production-observed'].includes(upgradeFrom), 'Supported upgrade states: 0015, complete 0025 or production-observed simulation');
...
function sql(text, database = db) {
  const result = spawnSync(psql, [...base, '-d', database], { input: text, encoding: 'utf8', env, maxBuffer: 32 * 1024 * 1024 });
  if (result.error || result.status !== 0) throw new Error(result.error?.message ?? result.stderr);
  return result.stdout.trim();
}
function concurrentSql(text) {
  return new Promise((resolve) => {
    const child = spawn(psql, [...base, '-d', db], { env }); let stdout = ''; let stderr = '';
    child.stdout.on('data', (value) => { stdout += value; }); child.stderr.on('data', (value) => { stderr += value; });
    child.on('error', (error) => resolve({ status: -1, stderr: error.message, stdout }));
    child.on('close', (status) => resolve({ status, stderr, stdout })); child.stdin.end(text);
  });
}
function nativeMigration(migration) {
  if (!native || migration.version !== '0018') return migration.sql;
  let content = migration.sql.replace(/^create extension if not exists (pg_cron|pgmq) with schema extensions;\r?\n/gm, '');
  let removed = 0;
...
try {
  ...
  if (native) {
    ...
    console.log('NATIVE LIMITATION: real PostgreSQL + upstream PGMQ; cron registration/worker and Supabase HTTP services are NOT tested.');
  }
  ...
  for (const migration of migrations) {
    ...
    if (upgradeFrom === 'production-observed' && migration.version === '0018') {
      ...
      for (const version of ['0020','0021','0022']) {
        ...
        assert(unrecorded); sql(`begin; ${unrecorded.sql} commit;`);
      }
    }
    if (upgradeFrom === migration.version || (upgradeFrom === 'production-observed' && migration.version === '0018')) {
  ...
  const fixtureResult = spawnSync(process.execPath,['--import','tsx',path.join(projectRoot,'scripts/database/catalog-fixture.ts')],
