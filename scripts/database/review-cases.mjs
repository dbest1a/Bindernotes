...
export async function runReviewCases({sql,concurrentSql,roleSql,as,users,json}) {
  ...
  const record={schemaVersion:1,recall:null,item:{id:'canonical-test',owner_id:users.a,type:'free_response',prompt:'What is conserved?',answer:'Mass',source_kind:'manual',
  ...
  const first=JSON.parse(as(users.a,call(record,0,[],operation)));assert.equal(first.revision,1);
  ...
  const event={id:'review-event',owner_id:users.a,item_id:record.item.id,rating:'good',reviewed_at:timestamp,due_at_before:timestamp,due_at_after:timestamp,response_length:7};
  assert.throws(()=>as(users.a,call({...record,item:{...record.item,prompt:'Must roll back'}},1,[{...event,owner_id:users.b}])),/INVALID_REVIEW_EVENT/);
  assert.equal(sql("select revision from public.review_items where owner_id='"+users.a+"' and id='canonical-test';"),'1');
  const competing=await Promise.all(Array.from({length:6},(_,i)=>concurrentSql(roleSql('authenticated',users.a,call({...record,item:{...record.item,prompt:`Writer ${i}`}},1)))));
  ...
  assert.throws(()=>as(users.a,call({...reviewed,item:{...reviewed.item,prompt:'History conflict'}},3,[{...event,rating:'forgot'}])),/REVIEW_HISTORY_CONFLICT/);
  assert.equal(sql(`select revision||':'||(payload->'item'->>'prompt') from public.review_items where owner_id='${users.a}' and id='canonical-test';`),'3:Reviewed');
  ...
  assert.throws(()=>as(users.a,call({...reviewed,item:{...reviewed.item,prompt:'Rolled back'}},3,[{...event,id:'rollback-event'}])),/TEST_REVIEW_EVENT_FAILURE/);
  assert.equal(sql(`select revision from public.review_items where owner_id='${users.a}' and id='canonical-test';`),'3');
  sql('drop trigger test_fail_review_event on public.review_events;drop function private.test_fail_review_event();');
  const session={id:'session',scope:{userId:users.a,binderId:'binder',documentId:'doc',lessonId:'lesson'},mode:'Flip',cardIds:['canonical-test'],correct:1,missed:0,score:100,missedConcepts:[],createdAt:timestamp};
  ...
  as(users.a,saveSession(session));as(users.a,saveSession(session));
