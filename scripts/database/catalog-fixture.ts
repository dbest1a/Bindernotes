...
import {
  ...
  mathSeedQuestions,
  mathSeedTopics,
} from "../../src/lib/math-learning-seeds";
...
if (!/^[\da-f]{8}-[\da-f]{4}-[\da-f]{4}-[\da-f]{4}-[\da-f]{12}$/i.test(operatorId))
  throw new Error("Expected disposable operator UUID");
const payload = buildSystemSeedPayload({
  id: operatorId,
  email: "admin@disposable.invalid",
  full_name: "Disposable operator",
  role: "admin",
