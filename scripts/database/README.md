...
`generate-types.mjs` introspects the actual disposable PostgreSQL catalog, never application row data. It generates68 public tables and63 RPC names through0039, including ordered columns/defaults/nullability, enum values, foreign-key relationships and unique-key cardinality. Function inputs admit SQLNULL because PostgreSQL arguments have no NOT NULL declaration; runtime domain validation and RPC guards still enforce required values. Table nullability remains exact. `--check` fails if the generated artifact differs. The disposable runtime suite runs that comparison against both clean and upgrade databases before teardown, so stale hand-maintained types are not silently accepted. To regenerate from the existing local Auth stack, set `BINDERNOTES_LOCAL_RUNTIME` and run `node scripts/database/generate-types.mjs`. The main browser Supabase client is typed; unknown editor/canvas payloads pass runtime JSON validation at write boundaries. JSONB domain schemas remain necessary because SQL only knows the JSON container type.
