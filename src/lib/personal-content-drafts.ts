...
const operationSchema = z.object({
  ...
  snapshot: personalContentSchema,
});
...
const journalSchema = z
  .object({ recordedAt: z.string().datetime(), draft: draftSchema })
  .superRefine((value, context) => {
    const draft = value.draft;
    const keyFor = (snapshot: PersonalContentSnapshot) =>
      `${snapshot.kind === "note" ? "personal-note" : snapshot.kind === "document" ? "personal-document" : "binder-note"}:${snapshot.id}`;
    if (
      draft.ownerId !== draft.snapshot.ownerId ||
      draft.entityKey !== keyFor(draft.snapshot) ||
      draft.savedRevision > draft.localRevision ||
      (draft.pending &&
        (draft.pending.snapshot.ownerId !== draft.ownerId ||
          keyFor(draft.pending.snapshot) !== draft.entityKey ||
          draft.pending.localRevision > draft.localRevision ||
          draft.pending.localRevision <= draft.savedRevision ||
          draft.pending.expectedRevision !== draft.serverRevision))
    )
...
export function createPersonalDraftJournal(ownerId: string, entityKey: string) {
  ...
  const list = (): DeviceBackup[] => {
    ...
    for (let index = 0; index < localStorage.length; index += 1) {
      ...
      try {
        ...
        if (
          parsed.success &&
          parsed.data.draft.ownerId === ownerId &&
          parsed.data.draft.entityKey === entityKey
        )
          backups.push({ key: candidate, ...parsed.data });
      } catch {
        /* Invalid backups remain untouched and are never rendered as document data. */
      }
    }
  ...
  return {
    read: () => {
      ...
      if (parsed.draft.ownerId !== ownerId || parsed.draft.entityKey !== entityKey)
