...
describe("detectMathSuggestions", () => {
  it("detects graphable expressions in note text", () => {
    const suggestions = detectMathSuggestions(emptyDoc("Let's compare y=x^2 and y=sin(x) in class."));
  ...
  it("detects latex-like expressions for reusable math blocks", () => {
    const suggestions = detectMathSuggestions(emptyDoc("Use \\int_0^1 x^2 dx for the warmup."));
  ...
  it("does not repeat expressions already stored as math blocks", () => {
    const suggestions = detectMathSuggestions(emptyDoc("Use y=x^2 in the note."), [
      {
        id: "graph-1",
        type: "graph",
        expressions: ["y=x^2"],
