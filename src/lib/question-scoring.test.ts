...
describe("scoreQuestion", () => {
  it.each([
    ...
    Infinity,
    -Infinity,
  ])("does not award a blank or invalid numeric response %j credit for zero", (numeric) => {
  ...
  it.each([
    ...
    { expected: 1, tolerance: NaN },
    { expected: 1, tolerance: Infinity },
  ...
  it("includes decimal tolerance boundaries without accepting nearby answers outside them", () => {
    for (const numeric of [5.99, 6.01]) {
      expect(
        scoreQuestion(question("numeric", { expected: 6, tolerance: 0.01 }), { numeric }).isCorrect,
      ).toBe(true);
    }
    expect(
      scoreQuestion(question("numeric", { expected: 0.3, tolerance: 0.1 }), { numeric: 0.4 }).isCorrect,
    ).toBe(true);
    expect(
      scoreQuestion(question("numeric", { expected: 6, tolerance: 0.01 }), { numeric: 6.010000001 })
        .isCorrect,
    ).toBe(false);
    expect(
      scoreQuestion(question("numeric", { expected: 1, tolerance: 0 }), { numeric: 1 + Number.EPSILON })
        .isCorrect,
    ).toBe(false);
    expect(
      scoreQuestion(question("numeric", { expected: 1e16, tolerance: 0.01 }), { numeric: 1e16 + 2 })
        .isCorrect,
    ).toBe(false);
  });
  ...
  it("normalizes multiple selections as sets without letting duplicates replace a missing answer", () => {
    ...
    expect(
      scoreQuestion(question("multiple_select", { correctChoiceIds: ["a", "a", "c"] }), {
  ...
  it.each(["multiple_choice", "multiple_select", "true_false", "step_ordering", "short_answer"] as const)(
    "does not award missing answers credit against an absent %s answer key",
    (type) => {
      expect(scoreQuestion(question(type, {}), {})).toMatchObject({
        autoGraded: false,
        isCorrect: null,
        pointsAwarded: null,
      });
  ...
  it("rejects malformed accepted text and empty completion responses", () => {
    expect(scoreQuestion(question("short_answer", { acceptedAnswers: [1] }), { text: "1" })).toMatchObject({
      autoGraded: false,
    });
    expect(
      scoreQuestion(question("free_response", { completionPoints: 1 }), { freeResponse: "   " })
        .pointsAwarded,
    ).toBeNull();
    expect(
      scoreQuestion(question("free_response", { completionPoints: Infinity }), { freeResponse: "A response" })
        .pointsAwarded,
    ).toBeNull();
  });

  it("scores numeric answers using tolerance", () => {
    const result = scoreQuestion(question("numeric", { expected: 6, tolerance: 0.01 }), { numeric: "6.005" });
  ...
  it("rejects numeric answers outside tolerance", () => {
    const result = scoreQuestion(question("numeric", { expected: 6, tolerance: 0.01 }), { numeric: "6.2" });
  ...
  it("requires exact multiple-select sets", () => {
    const correct = scoreQuestion(question("multiple_select", { correctChoiceIds: ["a", "c"] }), {
      selectedChoiceIds: ["c", "a"],
    });
    const extra = scoreQuestion(question("multiple_select", { correctChoiceIds: ["a", "c"] }), {
