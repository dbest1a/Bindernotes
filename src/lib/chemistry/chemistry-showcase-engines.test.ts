...
describe("chemistry showcase deterministic helpers", () => {
  it("finds elements by name, symbol, and atomic number", () => {
    ...
    expect(getTrendScore(findElement("O")!, "electronegativity")).toBeGreaterThan(
      getTrendScore(findElement("S")!, "electronegativity"),
    );
  });
  ...
  it("calculates acid-base, titration, kinetics, and calorimetry data without network calls", () => {
    ...
    expect(
      generateStrongAcidStrongBaseCurve({ acidMolarity: 0.1, acidVolumeMl: 25, baseMolarity: 0.1 }).some(
        (point) => point.equivalence,
      ),
    ).toBe(true);
    expect(
      generateKineticsDataset({ order: 1, initialConcentrationM: 1, rateConstant: 0.02 })[1].concentrationM,
    ).toBeLessThan(1);
    expect(
      calculateHeatTransfer({ massG: 100, specificHeatJPerGC: 4.184, deltaTemperatureC: 8 }),
    ).toBeCloseTo(3347.2);
  });
});
