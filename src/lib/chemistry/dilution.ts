...
export function calculateDilution(input: { stockM: number; targetM: number; targetVolumeMl: number }) {
  ...
  if (input.targetM > input.stockM)
    throw new RangeError("Target concentration cannot exceed stock concentration when diluting.");
  ...
  return {
    ...
    fractionOfStock: input.targetM / input.stockM,
  };
}
