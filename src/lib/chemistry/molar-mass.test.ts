...
describe("chemistry molar mass calculator", () => {
  it("computes molar masses from parsed atom inventories", () => {
    ...
    expect(calculateMolarMass("Ca(OH)2")).toMatchObject({
      ok: true,
      gramsPerMole: expect.closeTo(74.092, 3),
    });
    expect(calculateMolarMass("Al2(SO4)3")).toMatchObject({
      ok: true,
      gramsPerMole: expect.closeTo(342.131, 3),
    });
  });
