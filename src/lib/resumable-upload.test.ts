...
describe("resumable file transfer", () => {
    ...
    const request = vi.fn(async (_url: RequestInfo | URL, init?: RequestInit) => {
      if (init?.method === "POST")
        return new Response(null, {
          status: 201,
          headers: { Location: "https://storage.test/storage/v1/upload/resumable/id" },
        });
      if (init?.method === "HEAD")
        return new Response(null, {
          headers: { "Upload-Offset": String(remoteOffset), "Upload-Length": String(file.size) },
        });
      expect(new Headers(init?.headers).get("Upload-Offset")).toBe(String(remoteOffset));
      remoteOffset += (init?.body as Blob).size;
      if (fail) {
        fail = false;
        throw new TypeError("Network dropped after accepting chunk");
      }
      return new Response(null, { status: 204, headers: { "Upload-Offset": String(remoteOffset) } });
    });
    const options = {
      file,
      endpoint: "https://storage.test/storage/v1/upload/resumable",
      token: async () => "session",
      metadata: {},
  ...
  it("never sends session credentials to a foreign resume URL", async () => {
    ...
    await expect(
      uploadResumableFile(
        {
          file: new File(["data"], "test.pdf"),
          endpoint: "https://storage.test/storage/v1/upload/resumable",
          uploadUrl: "https://evil.test/upload",
          token,
          metadata: {},
          onLocation: vi.fn(),
        },
    ...
    ).rejects.toThrow("invalid resume address");
