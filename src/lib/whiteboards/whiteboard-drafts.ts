...
const boardSchema = z
  .object({
    ...
    title: z.string(),
    subject: z.string(),
    moduleContext: z.enum(["binder", "lesson", "math-lab"]),
    scene: z.object({
    ...
    modules: z.array(
      z.object({ id: z.string(), type: z.literal("bindernotes-module"), moduleId: z.string() }).passthrough(),
    ),
    revision: z.number().int().nonnegative().optional(),
    storageMode: z.enum(["local-draft", "supabase"]),
    objectCount: z.number(),
    sceneSizeBytes: z.number(),
    assetSizeBytes: z.number(),
    createdAt: z.string(),
    updatedAt: z.string(),
...
function identity(board: Pick<BinderWhiteboard, "id" | "ownerId">) {
  return `${encodeURIComponent(board.ownerId)}:${encodeURIComponent(board.id)}`;
}
function contextId() {
...
function parseDraft(raw: string): DurableDraft<BinderWhiteboard> {
  ...
  if (
    draft.entityKey !== draft.snapshot.id ||
    draft.ownerId !== draft.snapshot.ownerId ||
    draft.savedRevision > draft.localRevision ||
    (draft.pending &&
      (draft.pending.snapshot.id !== draft.entityKey ||
        draft.pending.snapshot.ownerId !== draft.ownerId ||
        draft.pending.localRevision > draft.localRevision))
  ) {
...
function storageFor(board: BinderWhiteboard, selectedBackup?: string): DraftStorage<BinderWhiteboard> {
  ...
  return {
    read: () => {
      ...
      const raw = localStorage.getItem(inherited);
...
saveQueue.subscribeAccount(() => {
  drafts.forEach((draft) => draft.retire());
  drafts.clear();
});
...
export function getWhiteboardDraft(board: BinderWhiteboard, selectedBackup?: string) {
  ...
  if (!draft) {
    ...
    try {
      storage = storageFor(board, selectedBackup);
    } catch {
      storage = {
        read: () => {
    ...
    draft = new RevisionedSave({
      ...
      entityKey: board.id,
      snapshot: board,
      serverRevision: board.revision ?? 0,
      storage,
      delay: AUTOSAVE_DEBOUNCE_MS,
      write: async (operation) => {
        if (saveQueue.getAccount() !== board.ownerId)
...
export function mergeWhiteboardDrafts(boards: BinderWhiteboard[], scope: WhiteboardScope) {
  ...
  const belongs = (board: BinderWhiteboard) =>
    board.ownerId === scope.ownerId &&
    board.binderId === scope.binderId &&
    board.lessonId === (scope.lessonId ?? null);
  try {
    const context = contextId();
    for (let index = 0; index < localStorage.length; index += 1) {
      const key = localStorage.key(index);
      if (!key?.startsWith(`${prefix}${encodeURIComponent(scope.ownerId)}:`) || !key.includes(`:${context}:`))
        continue;
      ...
      try {
        ...
        if (belongs(draft.snapshot) && sessionStorage.getItem(pointer) === key)
          recovered.set(draft.snapshot.id, draft.snapshot);
      } catch {
        /* Leave malformed backups untouched. */
      }
    }
  } catch {
    /* Visible controller errors report unavailable device storage. */
  }
  drafts.forEach((draft) => {
    const state = draft.getSnapshot();
    if (state.dirty && belongs(state.snapshot)) recovered.set(state.snapshot.id, state.snapshot);
  });
  boards
    .filter((board) => board.ownerId === scope.ownerId)
    .forEach((board) => {
  ...
  try {
    for (let index = 0; index < localStorage.length; index += 1) {
      ...
      try {
        ...
        result.push({ key, draft });
      } catch {
        /* Preserve unreadable backups. */
      }
    }
  } catch {
    /* The active controller reports storage failures. */
  }
  return result;
}

export function restoreWhiteboardBackup(board: BinderWhiteboard, backupKey: string) {
  if (!listWhiteboardBackups(board).some((backup) => backup.key === backupKey))
    throw new Error("This backup is unavailable.");
...
export function createWhiteboardDraftCopy(board: BinderWhiteboard) {
  ...
  const copy = {
    ...
    revision: 0,
    title: `${board.title} (draft copy)`,
    createdAt: now,
    updatedAt: now,
    archivedAt: null,
    storageMode: "local-draft" as const,
  };
