...
vi.mock("./whiteboard-storage", () => ({
  isScratchWhiteboard: (board: BinderWhiteboard) => board.id.startsWith("scratch-"),
...
function saved(snapshot: BinderWhiteboard, revision: number): WhiteboardSaveResult {
  return {
...
function backups() {
  return Object.keys(localStorage).filter((key) => key.startsWith("bindernotes:whiteboard-draft:v1:"));
}
beforeEach(() => {
  ...
  vi.mocked(saveWhiteboard)
    .mockReset()
    .mockImplementation(async (snapshot, options) => saved(snapshot, (options?.expectedRevision ?? 0) + 1));
});
afterEach(() => {
  saveQueue.setAccount(null);
  vi.useRealTimers();
...
describe("durable whiteboard drafts", () => {
  it("never creates a save controller from unloaded metadata and preserves a dirty scene across metadata refresh", () => {
    ...
    expect(
      mergeWhiteboardDrafts([metadata], { ownerId: "owner", binderId: "binder", lessonId: "lesson" }),
    ).toEqual([metadata]);
    const draft = getWhiteboardDraft(board());
    draft.edit((value) => ({ ...value, scene: { elements: [{ id: "unsaved-stroke" }] } }), false);
    expect(
      mergeWhiteboardDrafts([{ ...metadata, revision: 10 }], {
        ownerId: "owner",
        binderId: "binder",
        lessonId: "lesson",
      })[0].scene.elements,
    ).toEqual([{ id: "unsaved-stroke" }]);
  });
  it("captures immutable per-board scenes before the debounce and survives navigation", async () => {
    vi.useFakeTimers();
    const a = getWhiteboardDraft(board());
    const b = getWhiteboardDraft(board("board-b"));
    ...
    expect(saveWhiteboard).toHaveBeenCalledWith(
      expect.objectContaining({ id: "board-a", scene: { elements: [{ id: "stroke-a", version: 1 }] } }),
      expect.objectContaining({ expectedRevision: 4 }),
    );
    expect(saveWhiteboard).toHaveBeenCalledWith(
      expect.objectContaining({ id: "board-b", title: "B changed" }),
      expect.anything(),
    );
  });
  ...
  it("serializes revisions while edits arrive during an in-flight save", async () => {
    ...
    first.resolve(saved(board(), 5));
    await saving;
    expect(saveWhiteboard).toHaveBeenNthCalledWith(
      2,
  ...
  it("reuses the exact pending operation after uncertain failure and account rehydration", async () => {
    ...
    const draft = getWhiteboardDraft(board());
    draft.edit((current) => ({ ...current, title: "recover me" }), false);
    await draft.flush();
  ...
  it("preserves conflicts across remote refresh and offers an independent copy or explicit remote", async () => {
    vi.mocked(saveWhiteboard).mockResolvedValueOnce({
      ...saved(board(), 5),
      status: "conflict",
      message: "Conflict",
    });
    const draft = getWhiteboardDraft(board());
    draft.edit((current) => ({ ...current, title: "my changes" }), false);
    await draft.flush();
    expect(draft.getSnapshot().state).toBe("conflict");
    expect(
      mergeWhiteboardDrafts([{ ...board("board-a", 8), title: "someone else" }], {
        ownerId: "owner",
        binderId: "binder",
        lessonId: "lesson",
      })[0].title,
    ).toBe("my changes");
    const copy = createWhiteboardDraftCopy(draft.getSnapshot().snapshot);
    await copy.flush();
    expect(saveWhiteboard).toHaveBeenLastCalledWith(
      expect.objectContaining({ title: "my changes (draft copy)" }),
    ...
    expect(draft.getSnapshot()).toMatchObject({
      dirty: false,
      state: "saved",
      snapshot: { title: "someone else" },
    });
  });

  it("retains a late retired-account acknowledgement as a recoverable pending operation", async () => {
    const request = deferred<WhiteboardSaveResult>();
    vi.mocked(saveWhiteboard).mockReturnValueOnce(request.promise);
    const draft = getWhiteboardDraft(board());
    draft.edit((current) => ({ ...current, title: "pending" }), false);
    const saving = draft.flush();
    saveQueue.setAccount("other-owner");
    request.resolve(saved(board(), 5));
    await saving;
    expect(backups()).toHaveLength(1);
    expect(
      mergeWhiteboardDrafts([], { ownerId: "other-owner", binderId: "binder", lessonId: "lesson" }),
    ).toEqual([]);
  ...
  it("uses a new writer slot after reload or tab duplication and retains selectable inherited backups", async () => {
    const first = getWhiteboardDraft(board());
    first.edit((current) => ({ ...current, title: "first tab work" }), false);
    const inheritedKey = backups()[0];
    const inherited = localStorage.getItem(inheritedKey);
    // A cloned tab inherits the same session pointer, but initializes a distinct writer.
    saveQueue.setAccount(null);
    saveQueue.setAccount("owner");
    const second = getWhiteboardDraft(board());
    second.edit((current) => ({ ...current, title: "second tab work" }), false);
    expect(backups()).toHaveLength(2);
    expect(localStorage.getItem(inheritedKey)).toBe(inherited);
    await second.flush();
    expect(localStorage.getItem(inheritedKey)).toBe(inherited);
    expect(listWhiteboardBackups(board()).map((item) => item.draft.snapshot.title)).toContain(
      "first tab work",
    );
  ...
  it("does not automatically resurrect an inherited draft after an explicit remote choice", () => {
    const first = getWhiteboardDraft(board());
    first.edit((current) => ({ ...current, title: "old draft" }), false);
    saveQueue.setAccount(null);
    saveQueue.setAccount("owner");
    const recovered = getWhiteboardDraft(board());
    ...
    saveQueue.setAccount(null);
    saveQueue.setAccount("owner");
    expect(getWhiteboardDraft({ ...board(), title: "remote chosen" }).getSnapshot()).toMatchObject({
      dirty: false,
      snapshot: { title: "remote chosen" },
    });
