...
export type LessonSummary = Omit<LessonMetadata, "id"> & {
  lesson_id: string;
...
export function reconcileLessonSummaries(metadata: LessonMetadata[], summaries: LessonSummary[]) {
  ...
  return {
    ...
    repaired,
    removed,
    complete: repaired === 0 && removed === 0 && byId.size === summaries.length,
  };
}
