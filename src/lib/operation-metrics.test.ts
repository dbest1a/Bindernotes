...
import { clearOperationMetrics, flushOperationMetrics, inspectOperationMetrics, recordOperationMetric, setOperationMetricSink, timedSaveMetric } from "./operation-metrics";
const value = () => ({ release: "development", correlation: crypto.randomUUID(), operation: "content_save", result: "saved", durationMs: 25, retryCount: 0, payloadBytes: 500 });
afterEach(() => { clearOperationMetrics(); setOperationMetricSink(null); });
describe("content-free operation metrics", () => {
  ...
  it("measures bytes without retaining contents, entity IDs or exceptions", () => {
    ...
    expect(result).not.toContain("PRIVATE_SENTINEL"); expect(result).not.toContain("SECRET_ACCOUNT");
  ...
  it("bounds memory and isolates in-flight acknowledgement across account resets", async () => {
    ...
    setOperationMetricSink(() => new Promise<void>((resolve) => { finish = resolve; }));
    const report = flushOperationMetrics();
    clearOperationMetrics(); const next = value(); recordOperationMetric(next); finish?.(); await report;
  ...
  it("retains bounded measurements when reporting fails and never throws to the editor", async () => {
    setOperationMetricSink(async () => { throw new Error("provider secret"); });
