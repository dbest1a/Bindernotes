...
function buildTransferForge(context: AcceleratorContext): LearningAcceleratorCard {
  ...
  return card("transferForge", "Transfer Forge", "Same idea, new surface", "practice", [
...
function buildEvidenceLock(context: AcceleratorContext): LearningAcceleratorCard {
  ...
  return card("evidenceLock", "Evidence Lock", "Every important claim needs proof", "evidence", [
...
function buildMisstepMuseum(context: AcceleratorContext): LearningAcceleratorCard {
  return card("misstepMuseum", "Misstep Museum", "Save the almost-right answer", "repair", [
...
function buildConceptWeather(context: AcceleratorContext): LearningAcceleratorCard {
  const weather = context.highlightTexts.length > 2 && !context.noteText ? "foggy" : context.noteText ? "clearing" : "new";
  return card("conceptWeather", "Concept Weather", "Forecast what needs attention", "forecast", [
...
function buildRepresentationSwitchboard(context: AcceleratorContext): LearningAcceleratorCard {
  const modes = context.subjectLabel === "History"
  ...
  return card("representationSwitchboard", "Representation Switchboard", "One idea, many forms", "practice", [
...
function buildExamGhostMode(context: AcceleratorContext): LearningAcceleratorCard {
  return card("examGhostMode", "Exam Ghost Mode", "Cold recall before comfort", "practice", [
...
function buildWhiteboardReplay(context: AcceleratorContext): LearningAcceleratorCard {
  return card("whiteboardReplay", "Whiteboard Replay", "Narrate the path, not just the answer", "practice", [
...
function buildPrereqXray(context: AcceleratorContext): LearningAcceleratorCard {
  ...
  return card("prereqXray", "Prereq X-Ray", "Find the hidden weak link", "forecast", [
...
function buildMemoryWeave(context: AcceleratorContext): LearningAcceleratorCard {
  ...
  return card("memoryWeave", "Memory Weave", "Connect this lesson to another one", "evidence", [
...
function buildOneMinuteLab(context: AcceleratorContext): LearningAcceleratorCard {
  const labSurface = context.graphExpressions.length > 0
  ...
  return card("oneMinuteLab", "One-Minute Lab", "A tiny experiment inside the lesson", "experiment", [
...
function inferPrerequisite(context: AcceleratorContext) {
  const combined = `${context.lessonTitle} ${context.lessonText} ${context.formulaLabels.join(" ")}`.toLowerCase();
