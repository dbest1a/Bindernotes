...
import {
  reconcileLessonSummaries,
  type LessonMetadata,
  type LessonSummary,
} from "./dashboard-summary-coverage";
const row = (id: string): LessonMetadata => ({
  id,
  binder_id: "binder",
  title: id,
  order_index: 0,
...
const summary = (id: string): LessonSummary => ({
  ...row(id),
  lesson_id: id,
  plain_text_excerpt: "Current excerpt",
});
describe("dashboard summary coverage", () => {
  ...
  it("discards stale excerpt/title, deleted summaries and wrong-binder rows", () => {
    const result = reconcileLessonSummaries(
      [row("a"), row("b")],
      [
        { ...summary("a"), updated_at: "2025-01-01", title: "Old" },
        { ...summary("b"), binder_id: "other" },
        summary("deleted"),
      ],
    );
