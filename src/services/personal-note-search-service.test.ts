...
describe("bounded private note text search", () => {
  it("collects all 401 matching identities in bounded pages without caching bodies", async () => {
    ...
    mocks.rpc.mockImplementation(async (_name, args) => ({
      data: records.slice(args.p_offset, args.p_offset + args.p_limit),
      error: null,
    }));
    const found = await searchPersonalNoteBodies("owner-a", "rare phrase");
    expect(found.size).toBe(401);
    expect(found.has("personal-note:400")).toBe(true);
    expect(mocks.rpc.mock.calls.map((call) => call[1])).toEqual(
      [0, 200, 400].map((p_offset) => ({ p_query: "rare phrase", p_offset, p_limit: 200 })),
    );
  });
  it("does not return partial search results when a later page fails", async () => {
    mocks.rpc.mockResolvedValueOnce({
      data: Array.from({ length: 200 }, (_, index) => ({ kind: "binder-note", id: String(index) })),
      error: null,
    });
  ...
  it("aborts retired query work and validates server rows", async () => {
    ...
    controller.abort();
    await expect(searchPersonalNoteBodies("owner-a", "phrase", controller.signal)).rejects.toThrow(
      "cancelled",
    );
    expect(mocks.rpc).not.toHaveBeenCalled();
    mocks.rpc.mockResolvedValueOnce({
      data: [{ kind: "personal-note", id: "one", content: "Unexpected body" }],
      error: null,
    });
