...
function client() {
  if (!supabase) throw new Error("Sign in to use private files.");
  return supabase;
}
async function token(ownerId: string) {
  const { data, error } = await client().auth.getSession();
  if (error || data.session?.user.id !== ownerId)
    throw new Error("The account changed. Sign in before resuming this upload.");
  return data.session.access_token;
}
async function endpoint(kind: "complete" | "remove", ownerId: string, id: string) {
  const response = await fetch(`/api/assets/${kind}`, {
    method: "POST",
    headers: { Authorization: `Bearer ${await token(ownerId)}`, "Content-Type": "application/json" },
    body: JSON.stringify({ id }),
  });
  const body = await response.json().catch(() => ({}));
  if (!response.ok)
    throw new Error(body.message ?? "The file service is unavailable. Your upload can be retried.");
...
export async function uploadPrivateAsset(
  ...
  try {
    ...
    if (
      !journal ||
      !/^[0-9a-f-]{36}$/i.test(journal.id) ||
      (options.assetId && journal.id !== options.assetId)
    )
      throw new Error();
  } catch {
    throw new Error(
      "This upload's resume information could not be read. Remove the pending file from Files before retrying.",
    );
  }
  const { data, error } = await client().rpc("reserve_user_asset", {
    p_id: journal.id,
    p_name: file.name,
  ...
  if (asset.owner_id !== ownerId || asset.id !== journal.id)
    throw new Error("Upload reservation belongs to another account.");
  ...
  if (!journal.uploaded) {
    await uploadResumableFile({
      ...
      metadata: {
        bucketName: asset.bucket_id,
        objectName: asset.storage_path,
        contentType: asset.mime_type,
        cacheControl: "3600",
      },
      onLocation: (location) => {
        journal.uploadUrl = location;
        localStorage.setItem(key, JSON.stringify(journal));
      },
      ...options,
    });
  ...
  if (result.status !== "ready" && result.status !== "staged")
    throw new Error("The file service returned an invalid completion status.");
  localStorage.removeItem(key);
...
export async function removePrivateAsset(asset: PrivateAsset) {
  ...
  localStorage.removeItem(
    `bindernotes:upload:${asset.owner_id}:${asset.import_batch_id ?? "standalone"}:${asset.id}`,
  );
  localStorage.removeItem(
    `bindernotes:upload:${asset.owner_id}:${asset.import_batch_id ?? "standalone"}:${asset.sha256}:${encodeURIComponent(asset.name)}`,
  );
}
export async function privateAssetDownloadUrl(asset: PrivateAsset, download = true) {
  ...
  const { data, error } = await client()
    .storage.from(asset.bucket_id)
    .createSignedUrl(asset.storage_path, 60, download ? { download: asset.name } : undefined);
  if (error) throw error;
