...
export const savedQuizAttemptSchema = z.object({
  id: z.string().min(1), quiz_set_id: z.string().min(1), user_id: z.string().min(1),
  started_at: timestamp, completed_at: timestamp.nullable(),
  score: z.number().finite().nonnegative().nullable(), total_points: z.number().finite().nonnegative().nullable(),
  metadata_json: jsonObject.nullable(),
});
export const savedQuestionAttemptSchema = z.object({
  id: z.string().min(1), quiz_attempt_id: z.string().min(1), question_id: z.string().min(1), user_id: z.string().min(1),
  submitted_answer_json: jsonObject, is_correct: z.boolean().nullable(),
  points_awarded: z.number().finite().nonnegative().nullable(), feedback_json: jsonObject.nullable(), created_at: timestamp,
});
export const savedQuestionSnapshotSchema = z.object({
  id: z.string(), title: z.string().nullable(), promptMarkdown: z.string(), promptLatex: z.string().nullable(),
  explanationMarkdown: z.string().nullable(), type: z.string(),
...
export function snapshotQuestion(question: QuestionBankItem): z.infer<typeof savedQuestionSnapshotSchema> {
  return {
    id: question.id, title: question.title, promptMarkdown: question.prompt_markdown, promptLatex: question.prompt_latex,
    explanationMarkdown: question.explanation_markdown, type: question.type,
...
export async function getQuizAttemptResults(input: {
  attemptId: string; quizId: string; ownerId: string; signal?: AbortSignal;
}): Promise<QuizAttemptResults | null> {
  if (!input.attemptId.trim() || !input.quizId.trim() || !input.ownerId.trim()) throw new Error("A signed-in account and saved attempt are required.");
  if (!supabase) throw new Error("Account storage is unavailable. Saved quiz results cannot be loaded.");
  input.signal?.throwIfAborted();
  const attemptQuery = supabase.from("quiz_attempts").select("*")
    .eq("id", input.attemptId).eq("quiz_set_id", input.quizId).eq("user_id", input.ownerId);
  ...
  if (!parsedAttempt.success) throw new Error("The saved quiz attempt has invalid data and could not be displayed.");
  const attempt = parsedAttempt.data;
  if (attempt.id !== input.attemptId || attempt.quiz_set_id !== input.quizId || attempt.user_id !== input.ownerId) {
  ...
  for (let offset = 0; ; offset += pageSize) {
    const answersQuery = supabase.from("question_attempts").select("*")
      .eq("quiz_attempt_id", input.attemptId).eq("user_id", input.ownerId)
    ...
    if (!parsedAnswers.success) throw new Error("The saved answers have invalid data and could not be displayed.");
    if (parsedAnswers.data.some((answer) => answer.quiz_attempt_id !== input.attemptId || answer.user_id !== input.ownerId)) {
