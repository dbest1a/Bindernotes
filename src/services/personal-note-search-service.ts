...
const matchSchema = z.object({ kind: z.enum(["personal-note", "personal-document", "binder-note"]), id: z.string().min(1) }).strict();
let generation = 0;
saveQueue.subscribeAccount(() => { generation += 1; });

/** Match identities are paged; note bodies never enter a collection or search cache. */
export async function searchPersonalNoteBodies(ownerId: string, text: string, signal?: AbortSignal): Promise<Set<string>> {
  const query = text.trim();
  if (!query) return new Set();
  if (query.length > 200) throw new Error("Use a search of 200 characters or fewer.");
  const client = supabase, started = generation;
  const active = () => {
    if (signal?.aborted) throw new DOMException("Search cancelled", "AbortError");
    if (!client || saveQueue.getAccount() !== ownerId || started !== generation) throw new Error("The search account changed.");
  };
  active();
  ...
  for (let offset = 0; offset < 10000; offset += 200) {
    ...
    const { data, error } = await request; active();
