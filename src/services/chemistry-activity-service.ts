...
export const chemistryNotebookSchema = z.object({
  hypothesis: z.string().max(20000),
  procedure: z.string().max(20000),
  dataTable: z.string().max(20000),
  calculations: z.string().max(20000),
  observations: z.string().max(20000),
  errorAnalysis: z.string().max(20000),
  conclusion: z.string().max(20000),
});
export const chemistryActivitySchema = z.discriminatedUnion("kind", [
  z.object({
    kind: z.literal("stoichiometry"),
    version: z.literal(1),
    template: z.literal("template-water-from-hydrogen"),
    givenQuantity: z
      .string()
      .max(200)
      .refine((value) => (parseFiniteDecimal(value) ?? 0) > 0),
    result: z.object({
  ...
  z.object({
    kind: z.literal("titration"),
    version: z.literal(1),
    state: z.object({
      labTemplateId: z.literal("acid-base-titration-strong-acid-base"),
      acidFormula: z.literal("HCl"),
      baseFormula: z.literal("NaOH"),
      // This activity currently supports this one built-in experiment only.
      acidMolarity: z.literal(0.1),
      acidVolumeMl: z.literal(25),
      baseMolarity: z.literal(0.1),
      titrantAddedMl: finite.min(0).max(50),
      ph: finite.min(0).max(14),
      measurements: z
        .array(
          z.object({
            titrantVolumeMl: finite.min(0).max(50),
            ph: finite.min(0).max(14),
            equivalenceProgress: finite.min(0).max(2),
          }),
        )
        .max(1000),
      notebook: chemistryNotebookSchema,
      updatedAt: timestamp,
    }),
  }),
...
export type SavedChemistryActivity = {
  id: string;
  ownerId: string;
  createdAt: string;
  snapshot: ChemistryActivity;
};
const tables = {
  stoichiometry: "user_chem_attempts",
  titration: "user_lab_runs",
  notebook: "user_lab_reports",
} as const;
const rowSchema = z
  .object({ id: z.string().uuid(), user_id: z.string().uuid(), created_at: timestamp })
  .passthrough();
const pageSize = 20;
...
function parseRecord(value: unknown, ownerId: string, kind: ChemistryActivityKind): SavedChemistryActivity {
  ...
  if (row.data.user_id !== ownerId)
    throw new Error("This chemistry activity is unavailable for this account.");
  const raw =
    kind === "stoichiometry"
      ? row.data.metadata
      : kind === "titration"
        ? row.data.checkpoint
        : row.data.final_result;
  const snapshot = chemistryActivitySchema.safeParse(raw);
  if (!snapshot.success || snapshot.data.kind !== kind)
    throw new Error(
      "This saved activity uses an unsupported or invalid format. Your current work has been kept.",
    );
...
export async function saveChemistryActivity(input: {
  ...
  if (!z.string().uuid().safeParse(input.ownerId).success || !z.string().uuid().safeParse(input.id).success)
    throw new Error("A signed-in account and valid save ID are required.");
  ...
  const payload =
    snapshot.kind === "stoichiometry"
      ? {
          ...common,
          problem_template_id: null,
          final_answer: snapshot.result.finalAnswer,
          step_summaries: snapshot.result.steps,
          mistake_tags: [],
          concept_tags: snapshot.result.conceptTags,
          metadata: snapshot,
        }
      : snapshot.kind === "titration"
        ...
        : {
            ...
            lab_run_id: null,
            hypothesis: snapshot.notebook.hypothesis,
            procedure: snapshot.notebook.procedure,
            calculations: snapshot.notebook.calculations,
            observations: snapshot.notebook.observations,
            error_analysis: snapshot.notebook.errorAnalysis,
            conclusion: snapshot.notebook.conclusion,
...
export async function listChemistryActivities(input: {
  ...
  if (!z.string().uuid().safeParse(input.ownerId).success)
    throw new Error("Sign in to load saved chemistry work.");
  ...
  if (error || !Array.isArray(data))
    throw new Error("Saved chemistry work could not be loaded. Please retry.");
  ...
  for (const row of data.slice(0, pageSize)) {
    ...
    if (!row || row.user_id !== input.ownerId)
      throw new Error("Saved chemistry work is unavailable for this account.");
    try {
      records.push(parseRecord(row, input.ownerId, input.kind));
    } catch {
      unsupportedCount += 1;
    }
  }
