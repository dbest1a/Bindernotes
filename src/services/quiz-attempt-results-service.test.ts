...
beforeEach(() => {
  ...
  database.from.mockReset().mockImplementation((table: string) => {
    if (table !== "quiz_attempts" && table !== "question_attempts")
    ...
    const execute = (single: boolean) => {
      ...
      const matches = (row: Row) =>
        database.bypassFilters ||
        (row.user_id === database.owner && filters.every(([key, value]) => row[key] === value));
      if (write) {
        if (write.row.user_id !== undefined && write.row.user_id !== database.owner)
        ...
        if (write.kind === "update") {
          ...
          if (existing && (write.kind === "insert" || existing.user_id !== database.owner))
            return { data: null, error: { message: "duplicate or denied" } };
    ...
    const query = {
      ...
      upsert: (row: Row) => {
        write = { kind: "upsert", row };
        return query;
      },
      update: (row: Row) => {
        write = { kind: "update", row };
        return query;
      },
...
describe("saved quiz attempt repository", () => {
  it("preserves a free response without an optional rubric and omits absent answer fields", async () => {
    ...
    const saved = await submitQuestionAttempt({
      attemptId: attempt.id,
      userId: database.owner,
      question: prompt,
      answer: { freeResponse: "My proof", numeric: undefined },
    });
  ...
  it.each([NaN, Infinity, -Infinity])(
    "rejects a non-JSON numeric answer %s without storing a false null or zero",
    async (numeric) => {
      const attempt = await startQuizAttempt({ quizSetId: quiz.id, userId: database.owner });
      await expect(
        submitQuestionAttempt({
  ...
  it.each([NaN, Infinity, -1, 2])(
    "rejects invalid awarded points %s before storing a completion",
    async (pointsAwarded) => {
      const attempt = await startQuizAttempt({ quizSetId: quiz.id, userId: database.owner });
      await expect(
        completeQuizAttempt({
          ...
          quizSet: quiz,
          userId: database.owner,
          scores: [{ pointsAwarded, totalPoints: 1 }],
        }),
      ).rejects.toThrow(/invalid scoring data/);
  ...
  it("saves, completes, and reopens the exact attempt with unchanged answers, scores and original question context", async () => {
    ...
    const zero = await submitQuestionAttempt({
      attemptId: attempt.id,
      userId: database.owner,
      question: zeroQuestion,
      answer: { numeric: "" },
    });
    const correct = await submitQuestionAttempt({
      attemptId: attempt.id,
      userId: database.owner,
      question: question("twelve", 12),
      answer: { numeric: "12" },
    });
    await completeQuizAttempt({
      ...
      quizSet: quiz,
      userId: database.owner,
      scores: [zero.score, correct.score],
    });
    ...
    const reopened = await getQuizAttemptResults({
      attemptId: attempt.id,
      quizId: quiz.id,
      ownerId: database.owner,
    });
    expect(reopened?.attempt).toMatchObject({
      id: attempt.id,
      score: 1,
      total_points: 2,
      metadata_json: { quizTitle: "Saved algebra" },
    });
    expect(reopened?.attempt.id).not.toBe(other.id);
    expect(
      reopened?.answers.map((answer) => [
        answer.submitted_answer_json,
        answer.is_correct,
    ...
    expect(reopened?.answers[0].feedback_json?.questionSnapshot).toMatchObject({
      promptMarkdown: "Original zero question",
    });
  });
  ...
  it("retries a question write without creating a second answer record", async () => {
    ...
    expect(database.tables.question_attempts[0]).toMatchObject({
      is_correct: true,
      submitted_answer_json: { numeric: "1" },
    });
  });
  ...
  it("does not substitute another attempt, quiz, or account", async () => {
    const attempt = await startQuizAttempt({ quizSetId: quiz.id, userId: database.owner });
    expect(
      await getQuizAttemptResults({ attemptId: "missing", quizId: quiz.id, ownerId: database.owner }),
    ).toBeNull();
    expect(
      await getQuizAttemptResults({ attemptId: attempt.id, quizId: "other-quiz", ownerId: database.owner }),
    ).toBeNull();
    database.owner = "learner-b";
    expect(
      await getQuizAttemptResults({ attemptId: attempt.id, quizId: quiz.id, ownerId: "learner-b" }),
    ).toBeNull();
    expect(
      await getQuizAttemptResults({ attemptId: attempt.id, quizId: quiz.id, ownerId: "learner-a" }),
    ).toBeNull();
  });
  ...
  it("rejects a response containing another owner's attempt even if an upstream boundary malfunctions", async () => {
    ...
    await expect(
      getQuizAttemptResults({ attemptId: attempt.id, quizId: quiz.id, ownerId: "learner-b" }),
  ...
  it("paginates answers instead of reporting a partial response as the complete saved work", async () => {
    ...
    const saved = await submitQuestionAttempt({
      attemptId: attempt.id,
      userId: database.owner,
      question: question("one", 1),
      answer: { numeric: 1 },
    });
    database.tables.question_attempts = Array.from({ length: 251 }, (_, index) => ({
      ...saved.attempt,
      id: `row-${index}`,
  ...
  it("reports storage/invalid-data failures and respects aborted or superseded reads", async () => {
    ...
    await expect(getQuizAttemptResults({ ...input, signal: controller.signal })).rejects.toMatchObject({
      name: "AbortError",
    });
  });
});
...
function question(id: string, expected: number): QuestionBankItem {
  return {
    ...
    type: "numeric",
    title: null,
    prompt_markdown: `Original ${id} question`,
    prompt_latex: null,
    answer_json: { expected, tolerance: 0 },
    explanation_markdown: "Original explanation",
