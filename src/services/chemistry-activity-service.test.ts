  ...
  db.from.mockReset().mockImplementation((table: string) => {
    ...
    const execute = (single: boolean) => {
      ...
      if (payload) {
        ...
        filters.push(["id", row.id]);
      }
      const selected = rows
        .filter((row) => db.bypass || filters.every(([key, value]) => row[key] === value))
        .slice(from, to + 1);
...
describe("account-owned chemistry snapshots", () => {
  it("saves and reopens every lab measurement and notebook section without truncating to 80 rows", async () => {
    ...
    for (let step = 0; step < 100; step += 1)
      state = titrationReducer(state, { type: "add_titrant", volumeMl: 0.5 });
    state.notebook = {
      hypothesis: "Prediction",
      procedure: "100 half-mL steps",
      dataTable: "Readings",
      calculations: "n = cV",
      observations: "pH rose",
      errorAnalysis: "Overshoot",
      conclusion: "25 mL endpoint",
    };
    const snapshot: ChemistryActivity = {
      kind: "titration",
      version: 1,
  ...
  it("retains the original stoichiometry result, inputs, and steps with an idempotent retry", async () => {
    const snapshot: ChemistryActivity = {
      ...
      result: {
        ...
        finalAnswer: { value: 36.03, formula: "H2O", unit: "g" },
        steps: [{ kind: "given_to_moles", label: "4.032 g / 2.016 g/mol", value: 2, unit: "mol" }],
        conceptTags: ["mole_ratio"],
      },
    };
    ...
    expect(
      (await listChemistryActivities({ ownerId: ownerA, kind: "stoichiometry" })).records[0].snapshot,
    ).toEqual(snapshot);
  });
  it("keeps separate notebook reports and filters history by account", async () => {
    const snapshot: ChemistryActivity = {
      kind: "notebook",
      version: 1,
      notebook: { ...createTitrationInitialState().notebook, conclusion: "My private conclusion" },
    };
    await saveChemistryActivity({ id, ownerId: ownerA, snapshot });
    expect(db.tables.user_lab_reports[0]).toMatchObject({
      conclusion: "My private conclusion",
      lab_run_id: null,
    });
    expect(
      (await listChemistryActivities({ ownerId: ownerA, kind: "notebook" })).records[0].snapshot,
    ).toEqual(snapshot);
    expect((await listChemistryActivities({ ownerId: ownerB, kind: "notebook" })).records).toEqual([]);
    db.bypass = true;
    await expect(listChemistryActivities({ ownerId: ownerB, kind: "notebook" })).rejects.toThrow(
  ...
  it("rejects malformed checkpoints before attempting a write", async () => {
    const snapshot = {
      kind: "titration",
      version: 1,
      state: createTitrationInitialState(),
    } as ChemistryActivity;
    for (const ph of [NaN, Infinity, -1, 15]) {
      await expect(
        saveChemistryActivity({
          id,
          ownerId: ownerA,
          snapshot: { ...snapshot, state: { ...createTitrationInitialState(), ph } } as ChemistryActivity,
        }),
      ).rejects.toThrow(/Check the chemistry inputs/);
    }
    expect(db.from).not.toHaveBeenCalled();
  });
  it("does not replace failure with an empty success and preserves unsupported old records", async () => {
    db.fail = true;
    await expect(listChemistryActivities({ ownerId: ownerA, kind: "notebook" })).rejects.toThrow(
      /could not be loaded/,
    );
    await expect(
      saveChemistryActivity({
        id,
        ownerId: ownerA,
        snapshot: { kind: "notebook", version: 1, notebook: createTitrationInitialState().notebook },
      }),
    ).rejects.toThrow(/could not be saved/);
    db.fail = false;
    db.tables.user_lab_runs = [
      { id, user_id: ownerA, created_at: "2026-09-17T12:00:00Z", checkpoint: { legacy: true } },
    ];
    expect(await listChemistryActivities({ ownerId: ownerA, kind: "titration" })).toEqual({
      records: [],
      hasMore: false,
      unsupportedCount: 1,
    });
  ...
  it("pages history without silently dropping older work and cancels stale requests", async () => {
    db.tables.user_lab_reports = Array.from({ length: 21 }, (_, index) => ({
      id: `10000000-0000-4000-8000-${String(index).padStart(12, "0")}`,
      user_id: ownerA,
      created_at: "2026-09-17T12:00:00Z",
      final_result: { kind: "notebook", version: 1, notebook: createTitrationInitialState().notebook },
    }));
    ...
    expect(
      (await listChemistryActivities({ ownerId: ownerA, kind: "notebook", page: 1 })).records,
    ).toHaveLength(1);
    const controller = new AbortController();
    controller.abort();
    await expect(
      listChemistryActivities({ ownerId: ownerA, kind: "notebook", signal: controller.signal }),
    ).rejects.toThrow();
  });
});
