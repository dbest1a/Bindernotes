    ...
    from: (table: string) => {
      ...
      const query = {
        ...
        eq: (key: string, value: unknown) => {
          if (key === "owner_id") read.owner = value;
          if (key === "id") read.id = value;
          return query;
        },
        is: () => query,
        order: () => query,
        ...
        single: async () => {
          mocks.reads.push(read);
          return { data: mocks.rows.find((row) => row.id === read.id), error: null };
        },
        then: (resolve: (result: unknown) => unknown) => {
          const data =
            table === "personal_notes"
              ? mocks.rows
                  .slice(read.from, read.to + 1)
                  .map((row) => Object.fromEntries(read.select.split(",").map((key) => [key, row[key]])))
              : [];
          return Promise.resolve({
            data,
            error:
              mocks.failPage && table === "personal_notes" && read.from > 0 ? { message: "offline" } : null,
          }).then(resolve);
        },
      };
      return query;
...
vi.mock("@/services/binder-service", () => ({
  getDashboard: async () => ({ binders: [], lessons: [], folders: [], folderBinders: [] }),
...
beforeEach(() => {
  ...
  mocks.rows = Array.from({ length: 1201 }, (_, index) => ({
    id: `note-${index}`,
    owner_id: "owner",
    title: `Note ${index}`,
    content: emptyDoc("Large rich body"),
    math_blocks: [],
    tags: ["class"],
    folder_id: null,
    binder_id: null,
...
describe("metadata-first Personal Notes", () => {
  it("lists 1201 notes in bounded metadata pages and fetches only the selected body", async () => {
    ...
    expect(
      mocks.reads.every(
        (read) =>
          !read.select.includes("content") &&
          !read.select.includes("math_blocks") &&
          read.to - read.from === 199,
      ),
    ).toBe(true);
  ...
  it("reports a failed later page instead of presenting the first 200 as a complete list", async () => {
    ...
    expect(workspace.loadIssues).toEqual([
      expect.objectContaining({ table: "personal_notes", code: "personal_query_failed" }),
    ]);
  });
});
