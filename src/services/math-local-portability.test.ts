...
import {
  mathLocalArchiveSchema,
  mergeLocalMathArchive,
  readLocalMathArchive,
} from "./math-local-portability";
beforeEach(() => {
...
function seed() {
  const graph = saveStudyGraphLink({
    betaEnabled: true,
    ownerId: reviewOwnerA,
    mode: "2d",
    title: "Parabola",
    reflection: "Positive curvature",
    desmosState: {
      expressions: { list: [{ id: "a", latex: "y=x^2" }] },
      graph: { viewport: { xmin: -5, xmax: 5 } },
    },
    expressions: [{ latex: "y=x^2" }],
    noteId: "note-1",
  });
  const formula = createFormulaTheoremCard({
    betaEnabled: true,
    ownerId: reviewOwnerA,
    kind: "formula",
    title: "Power rule",
    statement: "nx^(n-1)",
    plainEnglishMeaning: "Multiply by exponent",
    exampleProblem: "x^2",
    commonMistake: "Forget coefficient",
    proofIdea: "Limit",
    graphLinkId: graph.id,
    reviewItemId: "cloud-card-1",
  });
  createProblemLogEntry({
    betaEnabled: true,
    ownerId: reviewOwnerA,
    problemTitle: "Derivative",
    source: "Section2",
    attempt: "x",
    finalAnswer: "2x",
...
it("refuses mismatched owners, duplicates, malformed JSON, nonfinite graph numbers and unsafe/deep data", () => {
  ...
  expect(
    mathLocalArchiveSchema.safeParse({
      ...archive,
      problemLogs: [...archive.problemLogs, ...archive.problemLogs],
    }).success,
  ).toBe(false);
  for (const state of [{ x: Infinity }, JSON.parse('{"__proto__":{"polluted":true}}'), { x: () => 1 }]) {
    expect(
      mathLocalArchiveSchema.safeParse({
        ...archive,
        graphLinks: [{ ...archive.graphLinks[0], desmos_state: state }],
      }).success,
    ).toBe(false);
  }
  let deep: unknown = {};
  ...
  expect(
    mathLocalArchiveSchema.safeParse({
      ...archive,
      graphLinks: [{ ...archive.graphLinks[0], desmos_state: deep }],
    }).success,
  ).toBe(false);
...
it("reports same-ID conflicts before writing any group", () => {
  ...
  const incoming = {
    ...archive,
    problemLogs: [{ ...archive.problemLogs[0], attempt: "Different device answer" }],
  };
  expect(mergeLocalMathArchive(reviewOwnerA, incoming)).toMatchObject({
    added: 0,
    conflicts: [{ kind: "problemLogs", id: archive.problemLogs[0].id }],
  });
