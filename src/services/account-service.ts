...
import { confirmAccountPassword } from "@/services/account-reauthentication";
...
export async function deleteOwnAccount(ownerId: string, confirmation: string, password?: string) {
  ...
  let confirmationSession: Awaited<ReturnType<typeof confirmAccountPassword>> | undefined;
  try {
    if (password) {
      const email = initial.data.session.user.email;
      if (!email) throw new Error("Sign in again to confirm deletion.");
      confirmationSession = await confirmAccountPassword(ownerId, email, password);
      check();
    }
    ...
    const response = await fetch("/api/account/delete", {
      ...
      headers: {
        ...
        Authorization: `Bearer ${confirmationSession?.accessToken ?? data.session.access_token}`,
      },
    ...
    await confirmationSession?.release();
  }
}
