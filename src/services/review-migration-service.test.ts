...
it("previews only exact owner data, rejects malformed/cross-account/orphan records and never alters originals", () => {
  ...
  window.localStorage.setItem(
    studyItemsStorageKey(reviewOwnerA),
    JSON.stringify([item, { ...item, owner_id: reviewOwnerB }, { ...item, due_at: "invalid" }]),
  );
  window.localStorage.setItem(
    studyItemsStorageKey(reviewOwnerB),
    JSON.stringify([studyFixture({ owner_id: reviewOwnerB })]),
  );
  window.localStorage.setItem(recallStorageKey(card), JSON.stringify([card]));
  window.localStorage.setItem(
    recallStorageKey({ ...card, userId: null }),
    JSON.stringify([{ ...card, userId: null }]),
  );
  const orphan = scheduleCanonicalReview(
    canonicalFromStudy(item),
    "good",
    new Date("2026-09-17T12:00:00Z"),
    "orphan",
  ).event;
  window.localStorage.setItem(
    studyReviewEventsStorageKey(reviewOwnerA),
    JSON.stringify([{ ...orphan, item_id: "missing" }]),
  );
...
it("imports source/schedule/history idempotently and leaves all device data available", async () => {
  const item = studyFixture();
  const transition = scheduleCanonicalReview(
    canonicalFromStudy(item),
    "hard",
    new Date("2026-09-17T12:00:00Z"),
    "history-1",
...
it("refuses to overwrite a different cloud record or import conflicting duplicate local IDs", async () => {
  ...
  await saveCanonicalReview({
    record: canonicalFromStudy({ ...item, answer: "Newer account answer" }),
    expectedRevision: 0,
    operationId: crypto.randomUUID(),
  });
  window.localStorage.setItem(studyItemsStorageKey(reviewOwnerA), JSON.stringify([item]));
  expect(
    await importLocalReviews(previewLocalReviewMigration(reviewOwnerA, window.localStorage)),
  ).toMatchObject({ saved: 0, conflicts: 1 });
  expect(database.tables.review_items[0].payload).toEqual(
    canonicalFromStudy({ ...item, answer: "Newer account answer" }),
  );
  window.localStorage.setItem(
    studyItemsStorageKey(reviewOwnerA),
    JSON.stringify([item, { ...item, answer: "Conflicting local answer" }]),
  );
  expect(previewLocalReviewMigration(reviewOwnerA, window.localStorage)).toMatchObject({
    records: [],
    issues: [{ key: "device", reason: expect.stringContaining("Conflicting") }],
  });
});

it("retains malformed JSON and reports failed saves without clearing the local source", async () => {
  const key = studyItemsStorageKey(reviewOwnerA);
  ...
  window.localStorage.setItem(key, JSON.stringify([studyFixture()]));
  database.fail = true;
  await expect(
    importLocalReviews(previewLocalReviewMigration(reviewOwnerA, window.localStorage)),
  ).rejects.toThrow();
