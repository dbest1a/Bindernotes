...
describe("typed study writes validate JSON before transport", () => {
  it("preserves valid graph JSON and supports expressions without an optional ID", async () => {
    const desmosState = {
      version: 11,
      expressions: { list: [{ id: "one", latex: "y=x^2" }] },
      graph: { viewport: { xmin: -2, xmax: 2 } },
    };
    await saveGraphState({
      ...
      calculatorMode: "2d",
      desmosState,
      expressions: [{ id: undefined, latex: "y=x^2" }],
    });
    expect(mocks.write).toHaveBeenCalledWith(
      expect.objectContaining({ desmos_state: desmosState, expressions: [{ latex: "y=x^2" }] }),
    );
  });
  it.each([NaN, Infinity, () => 1])("refuses invalid graph data before writing: %s", async (invalid) => {
    await expect(
      saveGraphState({ userId: "learner", title: "Invalid", calculatorMode: "2d", desmosState: { invalid } }),
    ).rejects.toThrow();
  ...
  it("rejects a non-finite answer key before altering a question or its choices", async () => {
    const input: QuestionInput = {
      userId: "learner",
      type: "numeric",
      promptMarkdown: "Invalid key",
      answerJson: { expected: Infinity },
