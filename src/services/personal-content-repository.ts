...
import {
  ...
  type PersonalContentSnapshot,
} from "@/lib/personal-content-contract";

function clientFor(ownerId: string) {
  if (!supabase || saveQueue.getAccount() !== ownerId)
    throw new Error("Sign in to the account that owns this draft before saving.");
  return supabase;
}
...
export async function savePersonalContent(operation: SaveOperation<PersonalContentSnapshot>) {
  ...
  const { data, error } = await clientFor(snapshot.ownerId).rpc("save_personal_content", {
    ...
    p_record: databaseJson({
      ...
      math_blocks: snapshot.mathBlocks,
      pinned: snapshot.pinned,
      binder_id: snapshot.binderId,
  ...
  if (error) {
    if (error.code === "40001" && error.message.includes("CONTENT_REVISION_CONFLICT"))
      throw new ContentConflictError();
    throw new Error(
      "The server could not save this note. Your draft is retained; retry when the connection is available.",
    );
  }
  const result = z
    .object({ id: z.string(), owner_id: z.string(), revision: z.number().int().nonnegative() })
    .parse(data);
  if (result.id !== snapshot.id || result.owner_id !== snapshot.ownerId)
...
const rowSchema = z.object({
  ...
  math_blocks: z.array(mathBlockSchema),
  pinned: z.boolean(),
...
export async function readPersonalContent(snapshot: PersonalContentSnapshot) {
  const table =
    snapshot.kind === "document"
      ...
      : snapshot.kind === "learner-note"
        ? "learner_notes"
        : "personal_notes";
  const { data, error } = await clientFor(snapshot.ownerId)
    .from(table)
    .select("*")
    .eq("owner_id", snapshot.ownerId)
    .eq("id", snapshot.id)
    .single();
  ...
  if (row.id !== snapshot.id || row.owner_id !== snapshot.ownerId)
    throw new Error("This note is not available to this account.");
  const content = personalContentSchema.parse({
    kind: snapshot.kind,
    id: row.id,
    ownerId: row.owner_id,
    title: row.title,
    content: row.content,
    mathBlocks: row.math_blocks,
...
export async function preservePersonalContentCopy(snapshot: PersonalContentSnapshot) {
  const copy: PersonalContentSnapshot = {
    ...snapshot,
    id: crypto.randomUUID(),
    kind: "note",
    title: `${snapshot.title || "Untitled note"} (recovered copy)`,
    folderId: null,
    binderId: null,
    documentId: null,
    lessonId: null,
  };
  await savePersonalContent({
    snapshot: copy,
    localRevision: 1,
    expectedRevision: 0,
