...
export function previewLocalReviewMigration(ownerId: string, storage: ReadStorage): LocalReviewMigration {
  const result: LocalReviewMigration = {
    ownerId,
    records: [],
    events: [],
    sessions: [],
  ...
  const queueKey = studyItemsStorageKey(ownerId),
    eventsKey = studyReviewEventsStorageKey(ownerId);
  ...
  for (let index = 0; index < storage.length; index += 1) {
    ...
    try {
      ...
      issue(key, "Invalid or oversized device data; original kept.");
      continue;
    }
    for (const raw of values as unknown[]) {
      if (key === queueKey) {
        ...
        if (!parsed.success || parsed.data.owner_id !== ownerId) {
          issue(key, "Invalid or mismatched review owner; original kept.");
          continue;
        }
        ...
        if (!parsed.success || parsed.data.owner_id !== ownerId) {
          issue(key, "Invalid or mismatched history owner; original kept.");
          continue;
        }
        result.events.push(parsed.data);
      } else if (key.endsWith(":cards")) {
        const parsed = recallCardSchema.safeParse(raw);
        if (!parsed.success || parsed.data.userId !== ownerId || recallStorageKey(parsed.data) !== key) {
          issue(key, "Invalid or mismatched recall scope; original kept.");
          continue;
        }
        ...
        if (
          !parsed.success ||
          parsed.data.scope.userId !== ownerId ||
          recallSessionStorageKey(parsed.data.scope) !== key
        ) {
          issue(key, "Invalid or mismatched session scope; original kept.");
          continue;
        }
  ...
  for (const record of result.records) {
    ...
    if (existing && JSON.stringify(existing) !== JSON.stringify(record)) {
      conflicts.add(record.item.id);
      issue("device", "Conflicting local review IDs require manual recovery; originals kept.");
    }
  ...
  result.events = result.events.filter((event) => {
    ...
    issue(eventsKey, "History has no matching valid local review item; original kept.");
    return false;
  });
  return result;
}
  ...
  for (const record of preview.records) {
    ...
    try {
      ...
      if (
        events.some(
          (event) =>
            eventsById.has(event.id) && JSON.stringify(eventsById.get(event.id)) !== JSON.stringify(event),
        )
      ) {
        report.conflicts += 1;
        continue;
      ...
      if (existing && missingEvents.length === 0) {
        report.unchanged += 1;
        continue;
      }
      if (missingEvents.length > 1000) {
        report.failed += 1;
        report.issues.push({
          key: "history",
          reason: "More than 1,000 events for one item need a larger reviewed import; original kept.",
        });
        continue;
      }
