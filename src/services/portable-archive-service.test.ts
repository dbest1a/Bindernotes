...
vi.mock("@/services/math-local-portability", async (original) => ({
  ...(await original<typeof import("@/services/math-local-portability")>()),
  mergeLocalMathArchive: mocks.math,
  readLocalMathArchive: (ownerId: string) => ({
    schemaVersion: 1,
    ownerId,
    problemLogs: [],
    formulaCards: [],
...
import {
  ...
  archiveImportCounts,
} from "./portable-archive-service";
beforeEach(() => {
  saveQueue.setAccount(owner);
  mocks.rpc.mockReset();
  mocks.upload.mockReset();
  mocks.existing = null;
  mocks.math.mockClear();
});
afterEach(() => {
...
const file = () =>
  new File([JSON.stringify(archiveFixture())], "workspace.json", { type: "application/json" });
describe("portable archive orchestration", () => {
  it("exports the full transaction snapshot and only this account's recoverable device journals", async () => {
    ...
    saveQueue.setAccount(archive.ownerId);
    mocks.rpc.mockResolvedValue({
      data: { tables: archive.tables, reviews: [], reviewEvents: [], recallSessions: [], assets: [] },
      error: null,
    });
    const entries = new Map([
      [
        `bindernotes:canonical-recall-draft:v1:${archive.ownerId}:draft`,
        JSON.stringify({ ownerId: archive.ownerId, draft: "Recall work" }),
      ],
      [
        `bindernotes:creator-draft:v1:${archive.ownerId}:lesson`,
        JSON.stringify({ ownerId: archive.ownerId, draft: "Creator work" }),
      ],
      [
        `bindernotes:creator-draft:v1:${owner}:lesson`,
        JSON.stringify({ ownerId: owner, draft: "Other account" }),
      ],
    ...
    expect(JSON.stringify(result)).toContain("Recall work");
    expect(JSON.stringify(result)).toContain("Creator work");
  ...
  it("preflights all content before writing and generates identical destination identities on retry", async () => {
    const first = await prepareArchiveImport(owner, file()),
      second = await prepareArchiveImport(owner, file());
    expect(first).toEqual(second);
    expect(first.archive.ownerId).toBe(owner);
    expect(mocks.rpc).not.toHaveBeenCalled();
    expect(mocks.upload).not.toHaveBeenCalled();
    await expect(
      prepareArchiveImport(owner, new File(['{"version":77}'], "unsupported.json")),
    ).rejects.toThrow();
    expect(mocks.rpc).not.toHaveBeenCalled();
  });
  it("reuses the transaction receipt after an uncertain response without another mutation", async () => {
    ...
    expect(await importPortableArchive(prepared)).toMatchObject({
      counts,
      recoveredBefore: true,
      deviceMathReady: true,
    });
    expect(mocks.rpc).toHaveBeenCalledTimes(1);
    expect(mocks.rpc.mock.calls[0][1]).toMatchObject({
      p_batch_id: prepared.batchId,
      p_archive_digest: prepared.digest,
    });
  });
  it("checks file bytes and hash before uploading any file or writing content", async () => {
    ...
    await expect(
      prepareArchiveImport(owner, new File([JSON.stringify(archive)], "archive.json")),
  ...
  it("stages verified files under new owner IDs and promotes only metadata with the transaction", async () => {
    ...
    mocks.upload.mockImplementation(async (ownerId, _file, options) => ({
      id: options.assetId,
      owner_id: ownerId,
      status: "staged",
    ...
    expect(mocks.upload).toHaveBeenCalledWith(owner, expect.any(File), {
      batchId: prepared.batchId,
      assetId: prepared.archive.assets[0].id,
    });
    expect(mocks.rpc.mock.calls[0][1].p_archive.assets[0]).not.toHaveProperty("base64");
    expect(mocks.rpc.mock.calls[0][1].p_archive.assets[0]).toMatchObject({
      id: prepared.archive.assets[0].id,
      sha256,
    });
  });
  it("does not claim device setup is complete if local storage cannot be restored", async () => {
    const prepared = await prepareArchiveImport(owner, file());
    mocks.rpc.mockResolvedValue({ data: archiveImportCounts(prepared.archive), error: null });
    mocks.math.mockImplementationOnce(() => {
