...
import { createCloudStudyItem, listCanonicalReviews, listCloudReviewEvents, recordCloudStudyReview, retryPendingReviewSaves, saveCanonicalReview } from "@/services/canonical-review-service";
...
beforeEach(() => {
  window.localStorage.clear(); database = reviewCloudFixture(); saveQueue.setAccount(reviewOwnerA);
  mocks.from.mockReset().mockImplementation(database.from); mocks.rpc.mockReset().mockImplementation(database.rpc);
});
...
describe("canonical review repository", () => {
  ...
  it("records one canonical rating and immutable history while keeping Recall's schedule consistent", async () => {
    ...
    await saveCanonicalReview({ record, expectedRevision: 0, operationId });
    const result = await recordCloudStudyReview({ ownerId: reviewOwnerA, itemId: record.item.id, rating: "good", response: "private answer" });
    ...
    expect(fresh.record.recall).toMatchObject({ reviewCount: 3, nextReviewAt: result.item.due_at, confidence: result.item.mastery });
    const history = await listCloudReviewEvents(reviewOwnerA);
    expect(history).toHaveLength(1); expect(history[0].response_length).toBe(14);
  ...
  it("retries an uncertain rating after reload without adding another event or moving the due date twice", async () => {
    ...
    await expect(recordCloudStudyReview({ ownerId: reviewOwnerA, itemId: record.item.id, rating: "hard" })).rejects.toThrow(/could not be saved/);
  ...
  it("does not create duplicate source cards when the first acknowledgement is lost", async () => {
    const input = { ownerId: reviewOwnerA, betaEnabled: true, prompt: "P", answer: "A", sourceKind: "manual" as const, type: "free_response" as const };
    database.loseNextAcknowledgement = true;
  ...
  it("isolates account data and rejects cross-account responses even if the transport misbehaves", async () => {
    await saveCanonicalReview({ record: canonicalFromStudy(studyFixture()), expectedRevision: 0, operationId });
    saveQueue.setAccount(reviewOwnerB); database.owner = reviewOwnerB;
  ...
  it("rejects stale writes, nonfinite schedules and malformed persisted data without overwriting", async () => {
    const record = canonicalFromStudy(studyFixture());
    await saveCanonicalReview({ record, expectedRevision: 0, operationId });
    await expect(saveCanonicalReview({ record: { ...record, item: { ...record.item, answer: "Stale" } }, expectedRevision: 0, operationId: crypto.randomUUID() })).rejects.toThrow(/another tab or device/);
    await expect(saveCanonicalReview({ record: { ...record, item: { ...record.item, mastery: NaN } }, expectedRevision: 1, operationId: crypto.randomUUID() })).rejects.toThrow();
