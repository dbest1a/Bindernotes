...
export function WhiteboardVersionHistory({
  board,
  expectedRevision,
  disabled,
  onRestored,
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    void listWhiteboardVersions(board)
      .then((items) => {
  ...
  async function restore(versionId: string) {
    ...
    const request =
      previous?.versionId === versionId && previous.revision === expectedRevision
        ? previous
        : { versionId, revision: expectedRevision, id: crypto.randomUUID() };
    ...
    try {
      ...
      await onRestored();
      setVersions(await listWhiteboardVersions(board));
      operation.current = null;
    } catch (cause) {
      setError(
        cause instanceof Error
          ? cause.message
          : "Restore failed. Your current board is preserved; retry when connected.",
      );
    } finally {
      setPending(false);
    }
  ...
  return (
    <details className="relative" open={open} onToggle={(event) => setOpen(event.currentTarget.open)}>
      ...
      {open && (
        <section
          aria-label="Whiteboard version history"
          className="absolute right-0 top-full z-50 mt-1 max-h-80 w-80 max-w-[85vw] overflow-auto rounded-lg border bg-background p-3 shadow-xl"
        >
          <p className="text-sm">
            The newest 50 automatic versions and explicit checkpoints are retained. Restoring creates a new
            version and keeps the chosen checkpoint.
          </p>
