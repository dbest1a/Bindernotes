...
vi.mock("@/components/theme/theme-provider", () => ({ ThemeProvider: ({ children }: { children: ReactNode }) => children }));
...
const tree = (owner: string) => <AuthenticatedAppProviders key={owner}><Probe /></AuthenticatedAppProviders>;
...
describe("authenticated query lifecycle", () => {
  ...
  it("loads the current account normally under StrictMode effect replay", async () => {
    ...
    render(<StrictMode><AuthenticatedAppProviders><QueryProbe /></AuthenticatedAppProviders></StrictMode>);
  ...
  it("cancels old queries and isolates late query and mutation results after account change", async () => {
    ...
    const pendingQuery = oldClient.fetchQuery({
      queryKey: ["late-private"], queryFn: () => new Promise<string>((resolve) => { resolveQuery = resolve; }),
    }).catch(() => undefined);
    const mutation = oldClient.getMutationCache().build(oldClient, {
      mutationFn: () => new Promise<string>((resolve) => { resolveMutation = resolve; }),
      onSuccess: (value) => { oldClient.setQueryData(["late-mutation"], value); },
    });
