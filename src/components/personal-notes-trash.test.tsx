...
vi.mock("@/services/personal-trash-service", () => ({
  listPersonalTrash: mocks.list,
  setPersonalTrash: mocks.change,
}));
...
function setup(dirty = false) {
  ...
  const view = render(
    ...
    >
      <PersonalNotesTrash ownerId="owner" data={undefined} hasUnsavedChanges={dirty} />
    </QueryClientProvider>,
  );
...
describe("Personal Notes trash controls", () => {
  it("restores directly but requires typed intent for permanent deletion", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Permanent deletion confirmation"), {
      target: { value: "DELETE" },
    });
  ...
  it("prevents destructive actions while an unsaved draft needs attention", async () => {
    setup(true);
    await screen.findByText("Chemistry");
    expect((screen.getByRole("button", { name: "Delete permanently" }) as HTMLButtonElement).disabled).toBe(
      true,
    );
