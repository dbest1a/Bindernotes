...
const test = vi.hoisted(() => ({
  ...
  rows: [] as SavedChemistryActivity[],
  save: vi.fn(),
  list: vi.fn(),
}));
vi.mock("@/hooks/use-auth", () => ({ useAuth: () => ({ profile: test.owner ? { id: test.owner } : null }) }));
vi.mock("@/services/chemistry-activity-service", () => ({
  saveChemistryActivity: test.save,
  listChemistryActivities: test.list,
}));
...
describe("chemistry save and reopen controls", () => {
  it("saves a titration and reopens its measurements and notebook after a remount", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Hypothesis"), {
      target: { value: "Expect equivalence at 25 mL." },
    });
  ...
  it("reopens stoichiometry original inputs and result, then computes again only after an edit", async () => {
    ...
    expect((screen.getByRole("button", { name: "Save chemistry work" }) as HTMLButtonElement).disabled).toBe(
      true,
    );
  });
  it("preserves a failed notebook save and retries the same operation without duplicate snapshots", async () => {
    ...
    expect(test.rows[0].snapshot).toMatchObject({
      notebook: { dataTable: "0 mL: pH 1", conclusion: "My conclusion" },
    });
  });
  it("keeps edits made during a save marked unsaved and prevents a duplicate click", async () => {
    let finish!: (value: unknown) => void;
    test.save.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
          finish = resolve;
        }),
