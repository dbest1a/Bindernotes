...
import { savedQuestionSnapshotSchema, type QuizAttemptResults, type SavedQuestionAttempt } from "@/services/quiz-attempt-results-service";

export function SavedQuizResults({ results }: { results: QuizAttemptResults }) {
  const { attempt, answers } = results;
  const title = typeof attempt.metadata_json?.quizTitle === "string" ? attempt.metadata_json.quizTitle : "Quiz";
  return <main className="app-page max-w-[1120px]">
    <Card>
      <CardHeader>
        <CardTitle>{title} results</CardTitle>
        <CardDescription>Saved attempt from {new Date(attempt.started_at).toLocaleString()}</CardDescription>
      </CardHeader>
...
function SavedAnswer({ answer }: { answer: SavedQuestionAttempt }) {
  ...
  return <article className="rounded-lg border border-border/70 bg-background/75 p-4">
    <div className="flex flex-wrap gap-2">
      <Badge variant={answer.is_correct ? "secondary" : "outline"}>{answer.is_correct === null ? "Saved for review" : answer.is_correct ? "Correct" : "Review"}</Badge>
      <span>{answer.points_awarded ?? "Not graded"}{typeof total === "number" && Number.isFinite(total) ? ` / ${total}` : " points"}</span>
    </div>
    <h3 className="mt-2 font-semibold">{snapshot?.title || snapshot?.promptMarkdown || `Question ${answer.question_id}`}</h3>
    {snapshot?.title && snapshot.promptMarkdown ? <p className="mt-2 whitespace-pre-wrap">{snapshot.promptMarkdown}</p> : null}
    {snapshot?.promptLatex ? <p className="mt-2 whitespace-pre-wrap">{snapshot.promptLatex}</p> : null}
    {!snapshot ? <p className="mt-2 text-sm text-muted-foreground">The original question text was not saved with this older attempt.</p> : null}
    ...
    {snapshot?.explanationMarkdown ? <p className="mt-3 whitespace-pre-wrap text-sm text-muted-foreground">{snapshot.explanationMarkdown}</p> : null}
  </article>;
}

export function formatSavedAnswer(answer: Record<string, unknown>, choiceLabels = new Map<string, string>()): string {
  if (typeof answer.numeric === "string" || typeof answer.numeric === "number") return String(answer.numeric).trim() || "No answer submitted";
  if (typeof answer.booleanAnswer === "boolean") return answer.booleanAnswer ? "True" : "False";
  if (typeof answer.selectedChoiceId === "string") return choiceLabels.get(answer.selectedChoiceId) ?? answer.selectedChoiceId;
  ...
  if (Array.isArray(selection) && selection.every((value: unknown) => typeof value === "string")) {
    return selection.map((value: string) => choiceLabels.get(value) ?? value).join(", ") || "No answer submitted";
  }
