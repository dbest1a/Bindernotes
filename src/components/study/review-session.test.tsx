...
const event: StudyReviewEvent = { id: "event", owner_id: item.owner_id, item_id: item.id, rating: "hard", reviewed_at: "2026-09-17T12:00:00.000Z", due_at_before: item.due_at, due_at_after: "2026-09-18T12:00:00.000Z", response_length: 2 };
it("keeps the answer on failed save and advances once after a confirmed retry", async () => {
  ...
  const onRate = vi.fn().mockRejectedValueOnce(new Error("Offline; retry"))
    .mockImplementationOnce(() => new Promise<StudyReviewEvent>((done) => { resolve = done; }));
