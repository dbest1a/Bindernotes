...
type Editor = {
  save: RevisionedSave<PersonalContentSnapshot>;
  journal: ReturnType<typeof createPersonalDraftJournal>;
  savedAt: string | null;
};
const editors = new Map<string, Editor>();
saveQueue.subscribeAccount(() => {
  editors.forEach((editor) => editor.save.retire());
  editors.clear();
});
const emptySnapshot: PersonalContentSnapshot = {
  kind: "learner-note",
  id: "",
  ownerId: "",
  title: "",
...
export function useLearnerNoteEditor(options: {
  ...
  ready: boolean;
  write: (input: LearnerNoteWrite) => Promise<LearnerNote>;
}) {
  const scope =
    options.ownerId && options.binderId && options.lessonId
      ? `${options.ownerId}:${options.binderId}:${options.lessonId}`
      : "";
  const [choiceError, setChoiceError] = useState<string | null>(null);
  const [adoptedNote, setAdoptedNote] = useState<{ scope: string; note: LearnerNote } | null>(null);
  const writerRef = useRef(options.write);
  writerRef.current = options.write;
  const editor = useMemo(() => {
    ...
    const snapshot = initialNote
      ? snapshotFromNote(initialNote, options.ownerId)
      : {
          ...emptySnapshot,
          id: initialId(scope, null),
          ownerId: options.ownerId,
          binderId: options.binderId,
          lessonId: options.lessonId,
          folderId: options.folderId,
          title: `${options.lessonTitle} notes`,
        };
    let journal: ReturnType<typeof createPersonalDraftJournal>;
    try {
      journal = createPersonalDraftJournal(options.ownerId, `binder-note:${snapshot.id}`);
    } catch {
      journal = {
        clearSelection: () => {},
        list: () => [],
        read: () => {
          throw new Error("Device backup unavailable");
        },
        write: () => {
          throw new Error("Device backup unavailable");
        },
    ...
    value.save = new RevisionedSave({
      ownerId: options.ownerId,
      entityKey: `binder-note:${snapshot.id}`,
      snapshot,
      serverRevision: initialNote ? contentRevision(initialNote) : 0,
      storage: journal,
      delay: 700,
      write: async (operation) => {
        if (saveQueue.getAccount() !== operation.snapshot.ownerId)
          throw new Error("Sign in to the draft's account before saving.");
        const saved = await writer({
          id: operation.snapshot.id,
          binderId: operation.snapshot.binderId!,
          lessonId: operation.snapshot.lessonId!,
          folderId: operation.snapshot.folderId,
          title: operation.snapshot.title,
          content: operation.snapshot.content,
          mathBlocks: operation.snapshot.mathBlocks,
          pinned: operation.snapshot.pinned,
          expectedRevision: operation.expectedRevision,
          operationId: operation.operationId,
        });
        if (saved.owner_id !== operation.snapshot.ownerId || saved.id !== operation.snapshot.id)
          throw new Error("The saved note did not match this draft.");
  ...
  const state = useSyncExternalStore(
    editor?.save.subscribe ?? noopSubscribe,
    editor?.save.getSnapshot ?? getEmpty,
    getEmpty,
  );
  const lifetimeRef = useRef({ scope, active: true });
  if (lifetimeRef.current.scope !== scope) {
    lifetimeRef.current.active = false;
    lifetimeRef.current = { scope, active: true };
  }
  ...
  useEffect(() => {
    ...
    if (options.note.id !== editor.save.getSnapshot().snapshot.id && adoptedNote?.scope !== scope) {
      ...
      setAdoptedNote({ scope, note: options.note });
      return;
    }
  ...
  useEffect(() => {
    ...
    const online = () =>
      editors.forEach((item) => {
        void item.save.flush();
      });
    const signOut = (event: Event) => {
      if (editor)
        (event as CustomEvent<{ promises?: Promise<unknown>[] }>).detail?.promises?.push(editor.save.flush());
    };
    window.addEventListener("pagehide", flush);
    window.addEventListener("online", online);
    window.addEventListener(NOTE_SAVE_BEFORE_SIGN_OUT_EVENT, signOut);
    return () => {
      window.removeEventListener("pagehide", flush);
      window.removeEventListener("online", online);
      window.removeEventListener(NOTE_SAVE_BEFORE_SIGN_OUT_EVENT, signOut);
    };
  }, [editor]);
  const change = useCallback(
    (update: (snapshot: PersonalContentSnapshot) => PersonalContentSnapshot) => {
      if (
        !editor ||
        editors.get(scope) !== editor ||
        saveQueue.getAccount() !== options.ownerId ||
        !lifetime.active ||
        lifetime !== lifetimeRef.current
      )
        return;
      setChoiceError(null);
      editor.save.edit(update);
    },
  ...
  const useRemote = async () => {
    ...
    try {
      ...
      if (remote.id !== snapshot.id) {
        editor.save.retire();
        editors.delete(scope);
        setAdoptedNote({ scope, note: remote });
      } else {
      ...
      if (lifetime.active)
        setChoiceError(error instanceof Error ? error.message : "Could not load saved note.");
    }
  };
  const preserveCopy = async () => {
    if (!editor) return;
    try {
      await preservePersonalContentCopy(editor.save.getSnapshot().snapshot);
      await useRemote();
    } catch (error) {
      if (lifetime.active)
        setChoiceError(error instanceof Error ? error.message : "Could not preserve draft copy.");
    }
  };
  const preserveBackup = async (key: string) => {
    ...
    try {
      await preservePersonalContentCopy(backup.draft.snapshot);
    } catch (error) {
      if (lifetime.active)
        setChoiceError(error instanceof Error ? error.message : "Could not preserve backup.");
    }
  };
  return {
    ...state,
    error: choiceError ?? state.error,
    ...
    mathBlocks: state.snapshot.mathBlocks,
    ...
    setMathBlocks: useCallback(
      (update: MathBlock[] | ((current: MathBlock[]) => MathBlock[])) =>
        change((value) => ({
          ...value,
          mathBlocks: typeof update === "function" ? update(value.mathBlocks) : update,
        })),
      [change],
    ),
    save: editor?.save.flush ?? (async () => {}),
    useRemote,
    preserveCopy,
    backups: editor?.journal.list() ?? [],
    preserveBackup,
  };
}
