...
function writer() {
  return vi.fn<(input: LearnerNoteWrite) => Promise<LearnerNote>>(async (input) => ({
    ...note(input.lessonId, input.title, input.expectedRevision + 1),
    id: input.id,
    content: input.content,
    math_blocks: input.mathBlocks,
  }));
}
function options(lesson: string, write = writer(), existing: LearnerNote | null = note(lesson)) {
  return {
    ownerId: "owner",
    binderId: "binder",
    lessonId: lesson,
...
beforeEach(() => {
  ...
  vi.mocked(readLearnerNoteByScope).mockReset();
});
afterEach(() => {
  cleanup();
  saveQueue.setAccount(null);
...
describe("reader lesson note saving", () => {
  it("persists each lesson before navigation and finishes its captured save after unmount", async () => {
    ...
    expect(a).toHaveBeenCalledWith(
      expect.objectContaining({ lessonId: "a", title: "A draft", expectedRevision: 3, pinned: true }),
    );
    expect(b).toHaveBeenCalledWith(
      expect.objectContaining({ lessonId: "b", title: "B draft", expectedRevision: 3 }),
    );
  });
  ...
  it("keeps dirty content across same-user hydration and ignores another lesson's late acknowledgement", async () => {
    ...
    const a = writer().mockImplementationOnce(
      () =>
        new Promise<LearnerNote>((done) => {
          resolve = done;
        }),
    );
    const hook = renderHook((props) => useLearnerNoteEditor(props), { initialProps: options("a", a) });
    ...
    hook.rerender(options("b", writer()));
    act(() => hook.result.current.setTitle("B draft"));
    await act(async () => {
      resolve(note("a", "A draft", 4));
      await saving;
    });
  ...
  it("preserves the pending operation and expected revision through uncertain retry and reload", async () => {
    ...
    act(() => hook.result.current.setTitle("Recover me"));
    await act(async () => hook.result.current.save());
    const operation = write.mock.calls[0][0];
    hook.unmount();
    act(() => {
      saveQueue.setAccount(null);
      saveQueue.setAccount("owner");
    });
    const reload = renderHook(() =>
      useLearnerNoteEditor(options("a", write, note("a", "Remote acknowledged", 4))),
    );
  ...
  it("keeps conflict explicit during edits until the user loads the remote version", async () => {
    ...
    const hook = renderHook(() => useLearnerNoteEditor(options("a", write)));
    act(() => hook.result.current.setTitle("My changes"));
    await act(async () => hook.result.current.save());
    expect(hook.result.current.state).toBe("conflict");
    act(() => hook.result.current.setTitle("More local changes"));
    await act(async () => hook.result.current.save());
  ...
  it("can explicitly adopt a concurrently created note with another ID", async () => {
    ...
    act(() => hook.result.current.setTitle("First note from this tab"));
    await act(async () => hook.result.current.save());
    ...
    act(() => hook.result.current.setTitle("Continue editing"));
    await act(async () => hook.result.current.save());
