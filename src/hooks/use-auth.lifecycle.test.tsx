...
const session = (id: string) =>
  ({ access_token: `token-${id}`, user: { id, email: `${id}@example.test` } }) as Session;
...
describe("auth account lifecycle", () => {
  ...
  it.each(["resolve", "reject"] as const)(
    "ignores a profile refresh that %ss after logout",
    async (outcome) => {
      const view = mount();
  ...
  it.each(["resolve", "reject"] as const)(
    "ignores an initial session read that %ss after a newer auth event",
    async (outcome) => {
  ...
  it("does not sign out B or unblock B hydration when A's save-before-sign-out finishes late", async () => {
    ...
    act(() => {
      signOut = view.result.current.signOut();
    });
  ...
  it("does not log out an account on network failure and allows a deleting session to retry", async () => {
    const view = mount();
    await waitFor(() => expect(view.result.current.profile?.id).toBe("A"));
    mocks.rpc.mockRejectedValueOnce(new TypeError("offline"));
    await act(async () => {
      await view.result.current.validateSession();
    });
    expect(view.result.current.user?.id).toBe("A");
    expect(view.result.current.profile?.id).toBe("A");
    expect(view.result.current.sessionCheckMessage).toMatch(/could not be checked/);
    mocks.rpc.mockResolvedValue({ data: "deleting", error: null });
    await act(async () => {
      await view.result.current.validateSession();
    });
    expect(view.result.current.user?.id).toBe("A");
  ...
  it("ignores revoked A evidence that arrives after B signs in", async () => {
    ...
    act(() => {
      check = view.result.current.validateSession();
    });
    await waitFor(() => expect(mocks.rpc).toHaveBeenCalled());
    await emit("SIGNED_IN", session("B"));
    await waitFor(() => expect(view.result.current.profile?.id).toBe("B"));
    await act(async () => {
      pending.resolve({ data: "revoked", error: null });
      await check;
    });
    expect(view.result.current.user?.id).toBe("B");
    expect(view.result.current.profile?.id).toBe("B");
    expect(mocks.signOut).not.toHaveBeenCalled();
  });
});
