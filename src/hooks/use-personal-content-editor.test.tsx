...
const entry = (title: string, revision: number, contentLoaded = true) =>
  ({
    id: "note",
    kind: "personal-note",
    title,
  ...
  }) as unknown as PersonalNotesEntry;
...
describe("metadata-safe personal editor", () => {
  it("waits for full content before creating the controller", () => {
    const { result, rerender } = renderHook(
      ({ selected }) => usePersonalContentEditor(selected, "owner", false),
      { wrapper, initialProps: { selected: entry("metadata", 3, false) } },
    );
  ...
  it("accepts a newer clean hydrated body but preserves unsaved edits across refetch", () => {
    const { result, rerender } = renderHook(
      ({ selected }) => usePersonalContentEditor(selected, "owner", false),
      { wrapper, initialProps: { selected: entry("first", 3) } },
    );
