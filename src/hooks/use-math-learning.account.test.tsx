...
vi.mock("@/hooks/use-auth", () => ({ useAuth: () => ({ profile: mocks.owner ? { id: mocks.owner } : null }) }));
...
describe("private math query ownership", () => {
  ...
  it.each(["question bank", "quiz"] as const)("does not show a late A %s response after switching to B", async (kind) => {
    let finishA!: (data: unknown) => void;
    mocks.read.mockImplementationOnce(() => new Promise((resolve) => { finishA = resolve; }));
    ...
    await act(async () => { finishA({ owner: "A", privateText: "A private" }); });
