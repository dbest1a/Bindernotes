...
describe("selected content hydration", () => {
  it("never exposes metadata as editable content, and ignores a late old selection", async () => {
    ...
    mocks.read.mockImplementation(
      (entry: PersonalNotesEntry) => new Promise((resolve) => pending.set(entry.id, resolve)),
    );
    ...
    const entry = (id: string) =>
      ({ id, kind: "personal-note", contentLoaded: false, updated_at: "1" }) as PersonalNotesEntry;
    const { result, rerender } = renderHook(({ selected }) => usePersonalEntryContent(selected, "owner"), {
      wrapper,
      initialProps: { selected: entry("a") },
    });
