...
import {
  contentRevision,
  personalContentFromEntry,
  type PersonalContentSnapshot,
} from "@/lib/personal-content-contract";
import {
  savePersonalContent,
  readPersonalContent,
  preservePersonalContentCopy,
} from "@/services/personal-content-repository";
...
export function usePersonalContentEditor(
  ...
  const editor = useMemo(() => {
    ...
    if (!existing) {
      ...
      let storage;
      try {
        storage = createPersonalDraftJournal(ownerId, identity);
      } catch {
        storage = {
          clearSelection: () => {},
          list: () => [],
      ...
      existing = new RevisionedSave({
        ...
        entityKey: identity,
        snapshot,
        serverRevision: contentRevision(entry.note),
        storage,
  ...
  useEffect(() => {
    ...
    if (revision > editor.getServerRevision())
      editor.useRemote(personalContentFromEntry(entry, ownerId), revision);
  ...
  const change = useCallback(
    (patch: Partial<PersonalContentSnapshot>) => {
      editor?.edit((current) => ({ ...current, ...patch }), autosave);
    },
    [editor, autosave],
  );
  ...
  const preserveCopy = useCallback(async () => {
    ...
    await queryClient.invalidateQueries({
      queryKey: queryKeys.personalNotes.forProfile(ownerId ?? undefined),
    });
    return copy;
  ...
  const preserveBackup = async (backupKey: string) => {
    const backup = journals
      .get(`${ownerId}:${identity}`)
      ?.list()
      .find((item) => item.key === backupKey);
    if (!backup || backup.draft.ownerId !== ownerId)
      throw new Error("This device backup is no longer available.");
