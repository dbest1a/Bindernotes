...
function PersonalNotesCreateDialog({
  ...
  const [binderId, setBinderId] = useState(selectedEntry?.personalBinderId ?? binders[0]?.id ?? "");
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (kind === "note") {
        ...
        onUpdatePreferences({ defaultNewNoteLocation: "loose" });
...
function NormalNotesView({
  ...
  return (
    ...
    >
      {!sidebarsHidden ? (
        <aside className="hidden min-h-0 w-[240px] shrink-0 overflow-auto border-r border-border/60 bg-background/82 p-2 lg:block 2xl:w-[280px]">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-sm font-semibold">Folders and binders</h2>
              ...
              <button
                className={`flex items-center justify-between rounded-md border px-3 py-2 text-left text-sm transition ${
                  folderFilter === folder.name
                    ...
                    : "border-border/70 bg-background/70 text-muted-foreground hover:bg-secondary/70 hover:text-foreground"
                }`}
                key={folder.name}
                onClick={() => onFolderFilter(folderFilter === folder.name ? null : folder.name)}
                type="button"
              >
          ...
          <div className="mt-6">
            <h3 className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Tags</h3>
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {tagSummaries.length === 0 ? (
                <p className="text-sm text-muted-foreground">Tags are optional and will show up here.</p>
              ) : null}
            </div>
          </div>
        </aside>
      ...
      {!sidebarsHidden ? (
        <section className="hidden min-h-0 w-[320px] shrink-0 overflow-hidden border-r border-border/60 bg-background/90 p-2 md:block 2xl:w-[360px]">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-sm font-semibold">Notes</h2>
            <Badge variant="outline">{entries.length}</Badge>
          </div>
          <div className="min-h-0 overflow-auto">
            <NoteList entries={entries} onSelectEntry={onSelectEntry} selectedEntry={selectedEntry} />
          </div>
        </section>
      ) : null}
...
function PersonalNotesSettingsDrawer({
  ...
  const sections = [
    ...
    },
    {
      ...
      label: "Notebook",
      terms:
        "notebook layout hide panes sidebar notebook notes list folder folders binder binders hierarchy drill all notes linked loose unfiled back up history math chemistry",
    },
    {
      id: "focus",
      label: "Editor state",
            ...
            <section className="rounded-xl border border-border bg-card p-4" key={section.id}>
              ...
              {section.id === "layout" ? (
                <div className="mt-3 grid gap-3">
                  ...
                  >
                    <option value="drill-in">Drill-in hierarchy</option>
                    <option value="expanded">Expanded hierarchy</option>
                  </SettingSelect>
                  <ToggleSetting
