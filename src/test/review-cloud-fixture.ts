...
export function recallFixture(overrides: Partial<RecallCard> = {}): RecallCard {
  return {
    ...
    sourceExcerpt: "A derivative is local change.",
    sourceAnchor: "paragraph-2",
    front: "Differentiate x^2.",
    back: "2x",
    explanation: "Power rule.",
    whyItMatters: "Local rate",
    tags: ["calculus"],
    subject: "Math",
    cardType: "formula_meaning",
    status: "Learning",
    difficulty: "good",
    confidence: 0.4,
    ...
    reviewCount: 2,
    lapseCount: 1,
    createdVia: "highlight",
    draftStatus: "accepted",
    qualityStatus: "good",
    missReasons: [
      { reason: "Careless error", note: "Forgot coefficient", createdAt: "2026-09-14T12:00:00.000Z" },
    ],
    teachBackReflections: [
      { text: "The exponent becomes the coefficient.", createdAt: "2026-09-15T12:00:00.000Z" },
    ],
...
export function studyFixture(overrides: Partial<StudyItem> = {}): StudyItem {
  return {
    ...
    source_kind: "formula",
    source_id: "formula-1",
    source_title: "Power rule",
    source_excerpt: "d/dx x^n = nx^(n-1)",
    binder_id: "binder-1",
...
export function reviewCloudFixture() {
  ...
  const fixture = {
    ...
    bypassOwner: false,
    tables: { review_items: [] as Row[], review_events: [] as Row[], review_sessions: [] as Row[] },
    receipts: new Map<string, { request: string; revision: number; id: string }>(),
  };
  function from(table: keyof typeof fixture.tables) {
    ...
    const read = (single: boolean) => {
      ...
      const rows = fixture.tables[table]
        .filter(
          (row) =>
            fixture.bypassOwner ||
            (row.owner_id === fixture.owner && filters.every(([key, value]) => row[key] === value)),
        )
    ...
    const query = {
      ...
      then: (resolve: (result: ReturnType<typeof read>) => unknown) =>
        Promise.resolve(read(false)).then(resolve),
    };
    return query;
  }
  async function rpc(name: string, args: Record<string, unknown>) {
    ...
    if (name === "save_review_session") {
      ...
      const existing = fixture.tables.review_sessions.find(
        (row) => row.id === args.p_id && row.owner_id === fixture.owner,
      );
      if (existing && JSON.stringify(existing.payload) !== JSON.stringify(session))
        return { data: null, error: { code: "40001" } };
      if (!existing)
        fixture.tables.review_sessions.push({
          id: args.p_id,
    ...
    const existing = fixture.tables.review_items.find(
      (row) => row.id === record.item.id && row.owner_id === fixture.owner,
    );
    if (prior) {
      if (prior.request !== request) return { data: null, error: { code: "22023" } };
      return prior.revision === existing?.revision
    ...
    fixture.tables.review_items = [
      ...fixture.tables.review_items.filter((candidate) => candidate !== existing),
      row,
    ];
    for (const event of (args.p_events ?? []) as Row[])
      if (
        !fixture.tables.review_events.some(
          (prior) => prior.id === event.id && prior.owner_id === fixture.owner,
        )
      )
        fixture.tables.review_events.push({
          id: event.id,
          owner_id: fixture.owner,
          payload: structuredClone(event),
        });
    fixture.receipts.set(key, { request, revision: row.revision, id: row.id });
    if (fixture.loseNextAcknowledgement) {
      fixture.loseNextAcknowledgement = false;
