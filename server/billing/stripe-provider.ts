    ...
    for (const payment of payments.data) {
      ...
      if (charge.disputed) {
        ...
        if (
          ...
          disputes.data.some((dispute) => idOf(dispute.charge) !== charge.id)
        )
          throw new Error("Dispute scope incomplete");
        disputeResolved = disputes.data.every((dispute) =>
          ["won", "warning_closed"].includes(dispute.status),
        );
      }
      if (charge.paid && !charge.refunded && disputeResolved)
        settled += Math.min(payment.amount_paid ?? 0, Math.max(0, charge.amount - charge.amount_refunded));
    }
  ...
  return {
    async verifyEvent(body, signature) {
      const event = await (async () => {
        try {
          return await stripe.webhooks.constructEventAsync(body, signature, options.webhookSecret);
        } catch {
          throw new BillingError(400, "Invalid webhook signature.");
        }
      })();
      const object = z
        .object({ customer: z.union([z.string(), z.object({ id: z.string() })]).nullish() })
      ...
      if (event.type.startsWith("charge.dispute.")) {
        const dispute = z
          .object({ charge: z.union([z.string(), z.object({ id: z.string() })]) })
          .parse(event.data.object);
    ...
    async createCustomer(ownerId, email) {
      const customer = await stripe.customers.create(
        { email, metadata: { bindernotes_user_id: ownerId } },
        { idempotencyKey: `bindernotes-customer:${ownerId}` },
      );
      return customer.id;
    },
    async subscriptions(customerId) {
      ...
      for (const subscription of subscriptions.data) {
        if (subscription.items.has_more || idOf(subscription.customer) !== customerId)
          throw new Error("Subscription scope incomplete");
    ...
    async checkout(customerId, plan, requestId) {
      ...
      if (
        !price.active ||
        price.livemode !== options.live ||
        price.currency !== "usd" ||
        price.unit_amount !== amounts[plan] ||
      ...
      for (const session of sessions.data) {
        if (idOf(session.customer) !== customerId || session.livemode !== options.live)
          throw new Error("Checkout scope mismatch");
      ...
      const session = await stripe.checkout.sessions.create(
        {
          ...
          success_url: `${options.origin}/pricing?billing=processing`,
          cancel_url: `${options.origin}/pricing?billing=canceled`,
    ...
    async portal(customerId) {
      const session = await stripe.billingPortal.sessions.create({
        customer: customerId,
        return_url: `${options.origin}/pricing`,
      });
      return session.url;
    },
  };
