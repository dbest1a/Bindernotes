...
export function supabaseBillingStore(client: SupabaseClient<Database>): BillingStore {
  ...
  return {
    ...
    async customer(ownerId) {
      ...
      return data
        ? z.object({ customer_id: z.string().regex(/^cus_[a-zA-Z0-9]+$/) }).parse(data).customer_id
        : null;
    },
    async bindCustomer(ownerId, customerId) {
      return z
        .string()
        .parse(await rpc("bind_billing_customer", { p_owner: ownerId, p_customer: customerId }));
    },
    async claim(customerId, eventId, token) {
      return z
        .enum(["claimed", "complete", "busy", "unmapped"])
        .parse(
          await rpc("claim_billing_event", { p_customer: customerId, p_event: eventId, p_token: token }),
        );
    },
    async finish(customerId, eventId, token, entitlement) {
      await rpc("finish_billing_event", {
        p_customer: customerId,
        p_event: eventId,
        p_token: token,
        p_entitlement: entitlement ? entitlementSchema.parse(entitlement) : null,
      });
    },
    async release(customerId, token) {
      await rpc("release_billing_lease", { p_customer: customerId, p_token: token });
    },
  };
}
