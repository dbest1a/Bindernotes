...
export type SubscriptionState = {
  id: string; priceId: string; status: string; periodEnd: number;
  paid: boolean; fullyRefunded: boolean;
};
...
export function deriveEntitlement(states: SubscriptionState[], prices: Record<PaidPlan, string>, now: number): Entitlement {
  ...
  for (const state of states) {
    ...
    if (!plan || state.status !== "active" || !state.paid || state.fullyRefunded || !Number.isFinite(state.periodEnd) || state.periodEnd <= now) continue;
    if (rank[plan] > rank[result.plan] || (plan === result.plan && state.periodEnd > Date.parse(result.validUntil ?? "") / 1000)) {
...
export interface BillingStore {
  ...
  claim(customerId: string, eventId: string, token: string): Promise<"claimed" | "complete" | "busy" | "unmapped">;
