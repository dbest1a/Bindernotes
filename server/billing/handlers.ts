...
import { BillingError, billingEvents, checkoutSchema, deriveEntitlement, type BillingProvider, type BillingStore, type PaidPlan } from "./contracts";

type Dependencies = { provider: BillingProvider; store: BillingStore; prices: Record<PaidPlan, string>; origin: string; live: boolean };
...
async function boundedText(request: Request, limit: number) {
  ...
  try {
    for (;;) {
      ...
      if (size > limit) { await reader.cancel(); throw new BillingError(413, "Request is too large."); }
      chunks.push(value);
    }
...
export function createBillingHandlers(deps: Dependencies) {
  async function authenticated(request: Request) {
    ...
    if (request.headers.get("origin") !== deps.origin) throw new BillingError(403, "Return to BinderNotes to manage your plan.");
  ...
  return {
    checkout: safe(async (request) => {
      ...
      let input: unknown;
      try { input = JSON.parse(await boundedText(request, 2048)); } catch (error) {
      ...
      const customerId = await deps.store.customer(user.id) ?? await deps.store.bindCustomer(user.id, await deps.provider.createCustomer(user.id, user.email));
      const eventId = `checkout:${requestId}`;
      const token = randomUUID();
      const claim = await deps.store.claim(customerId, eventId, token);
      if (claim === "busy") throw new BillingError(409, "Another billing change is still processing. Please retry shortly.");
      ...
      try {
        ...
        if (states.some((state) => !["canceled", "incomplete_expired"].includes(state.status))) {
          throw new BillingError(409, "You already have a subscription. Use Manage billing to change or cancel it.");
        }
        ...
        return json({ url });
      } finally { if (claim === "claimed") await deps.store.release(customerId, token); }
    }),
    ...
    webhook: safe(async (request) => {
      ...
      if (event.live !== deps.live) throw new BillingError(400, "Webhook mode does not match this environment.");
      ...
      try {
        ...
        await deps.store.finish(customerId, event.id, token, deriveEntitlement(current, deps.prices, Date.now() / 1000));
        return json({ received: true });
      } finally { await deps.store.release(customerId, token); }
    }),
  };
}
