...
const state = (override: Partial<SubscriptionState> = {}): SubscriptionState => ({
  id: "sub_1",
...
const request = (body: unknown, options: { path?: string; authorization?: string; origin?: string } = {}) =>
  new Request(`${origin}/api/billing/${options.path ?? "checkout"}`, {
    method: "POST",
    headers: {
      Origin: options.origin ?? origin,
      Authorization: options.authorization ?? "Bearer valid-a",
...
function setup() {
  ...
  const provider: BillingProvider = {
    ...
    createCustomer: vi.fn(async () => "cus_A"),
    subscriptions: vi.fn(async () => current),
    checkout: vi.fn(async () => "https://checkout.stripe.com/c/pay/test"),
    portal: vi.fn(async () => "https://billing.stripe.com/p/session/test"),
  };
  const store: BillingStore = {
    authenticate: vi.fn(async (token) => {
      if (token !== "valid-a") throw new BillingError(401, "Sign in again.");
      return { id: "owner-a" };
    }),
    customer: vi.fn(async () => "cus_A"),
    bindCustomer: vi.fn(async (_owner, customer) => customer),
    ...
    release: vi.fn(async (_customer, token) => {
  ...
  return {
    provider,
    store,
    handlers: createBillingHandlers({ provider, store, prices, origin, live: false }),
    setCurrent: (value: SubscriptionState[]) => {
      current = value;
    },
    setEvent: (value: BillingEvent) => {
      event = value;
    },
...
describe("trusted billing handlers", () => {
    ...
    expect(
      (
        await s.handlers.checkout(
          request({ plan: "studio", requestId, userId: "owner-b", priceId: "price_cheap" }),
        )
      ).status,
    ).toBe(400);
  ...
  it("rejects expired authentication and cross-origin requests", async () => {
    const s = setup();
    const input = { plan: "plus", requestId: crypto.randomUUID() };
    expect((await s.handlers.checkout(request(input, { authorization: "Bearer forged" }))).status).toBe(401);
    expect((await s.handlers.checkout(request(input, { origin: "https://attacker.example" }))).status).toBe(
      403,
    );
    expect(s.provider.checkout).not.toHaveBeenCalled();
  });
  it("prevents duplicate subscriptions and exposes the account's management portal", async () => {
    ...
    s.setCurrent([state()]);
    expect(
      (await s.handlers.checkout(request({ plan: "plus", requestId: crypto.randomUUID() }))).status,
    ).toBe(409);
    expect(s.provider.checkout).not.toHaveBeenCalled();
  ...
  it("deduplicates webhook delivery and grants product features without role mutation", async () => {
    const s = setup();
    s.setCurrent([state()]);
    await s.handlers.webhook(request({}));
    await s.handlers.webhook(request({}));
    expect(s.provider.subscriptions).toHaveBeenCalledTimes(1);
    expect(s.getEntitlement()).toEqual({
      plan: "studio",
      status: "active",
      validUntil: new Date(future * 1000).toISOString(),
    });
  });
  it("reversed event order cannot resurrect a canceled entitlement", async () => {
    const s = setup();
    s.setCurrent([state({ status: "canceled" })]);
  ...
  it("failed reconciliation retries without marking the event processed or exposing secrets", async () => {
    const s = setup();
    vi.mocked(s.provider.subscriptions).mockRejectedValueOnce(new Error("secret-customer-private"));
    const first = await s.handlers.webhook(request({}));
    expect(first.status).toBe(503);
    expect(await first.text()).not.toContain("secret-customer-private");
    s.setCurrent([state()]);
    expect((await s.handlers.webhook(request({}))).status).toBe(200);
  ...
  it("requests provider redelivery while a customer reconciliation is busy", async () => {
    const s = setup();
    vi.mocked(s.store.claim).mockResolvedValue("busy");
  ...
  it("refuses invalid signatures and the wrong test/live mode", async () => {
    const s = setup();
    vi.mocked(s.provider.verifyEvent).mockRejectedValueOnce(
      new BillingError(400, "Invalid webhook signature."),
    );
  ...
  it("bounds actual streamed input and fails closed when billing is unconfigured", async () => {
    ...
    try {
      expect((await billingEndpoint("checkout")(request({}))).status).toBe(503);
    } finally {
      vi.unstubAllEnvs();
    }
  });
...
describe("current provider state entitlement policy", () => {
  it.each(["past_due", "unpaid", "incomplete", "incomplete_expired", "canceled", "paused", "trialing"])(
    "removes paid access for %s",
    (status) => {
  ...
  it("handles renewal failure, full refund, untrusted prices and expired periods", () => {
    for (const value of [
      state({ paid: false }),
      state({ fullyRefunded: true }),
      state({ priceId: "price_unknown" }),
      state({ periodEnd: 0 }),
      state({ periodEnd: NaN }),
    ]) {
...
describe("official Stripe signature verification", () => {
  it("accepts an authentic raw body and rejects altered/expired signatures", async () => {
    ...
    expect(await provider.verifyEvent(payload, header)).toMatchObject({
      id: "evt_signed",
      customerId: "cus_A",
      live: false,
    });
    await expect(provider.verifyEvent(payload.replace("cus_A", "cus_B"), header)).rejects.toThrow(
