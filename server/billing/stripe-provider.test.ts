...
function fixture(routes: Record<string, unknown>) {
  ...
  return {
    ...
    provider: stripeProvider(stripe, {
      ...
      prices,
      live: false,
      webhookSecret: "whsec_disposable_fixture",
    }),
  };
}
const list = (data: unknown[], has_more = false) => ({ object: "list", data, has_more });
const session = (id: string, mode = "subscription") => ({
  id,
  ...
  mode,
  status: "open",
  metadata: { bindernotes_plan: "studio" },
...
describe("provider checkout reconciles actual open-session prices", () => {
  it("expires a matching-metadata session with an obsolete price and creates the configured subscription", async () => {
    ...
    expect(await s.provider.checkout("cus_A", "studio", "request-a")).toBe(
      "https://checkout.stripe.com/cs_new",
    );
  ...
  it("reuses only one exact-price subscription session and expires an extra duplicate", async () => {
    ...
    expect(await s.provider.checkout("cus_A", "studio", "request-a")).toBe(
      "https://checkout.stripe.com/cs_first",
    );
  ...
  it.each([list([line()], true), list([line(), line()]), list([line(prices.studio, 2)])])(
    "does not reuse incomplete or changed line items",
    async (lines) => {
      const s = fixture({
        "GET /v1/prices/price_studio": price,
...
function disputedPayment(statuses: string[], hasMore = false) {
  return fixture({
    "GET /v1/subscriptions": list([
      {
        ...
        status: "active",
        latest_invoice: "in_A",
        items: list([{ price: { id: prices.studio }, current_period_end: 2000000000 }]),
      },
    ]),
    ...
    "GET /v1/disputes": list(
      statuses.map((status, index) => ({ id: `du_${index}`, charge: "ch_A", status })),
      hasMore,
    ),
  });
}
describe("provider dispute reconciliation", () => {
  it.each(["won", "warning_closed"])(
    "restores paid eligibility after %s despite the historical disputed flag",
    async (status) => {
      const s = disputedPayment([status]);
      expect(await s.provider.subscriptions("cus_A")).toMatchObject([{ paid: true, fullyRefunded: false }]);
    },
  );
  it.each(["lost", "needs_response", "under_review", "warning_needs_response", "warning_under_review"])(
    "withholds disputed eligibility while %s",
    async (status) => {
      expect(await disputedPayment([status]).provider.subscriptions("cus_A")).toMatchObject([
        { fullyRefunded: true },
      ]);
    },
  );
  it("requires every dispute to be resolved and rejects incomplete provider pages", async () => {
    expect(await disputedPayment(["won", "lost"]).provider.subscriptions("cus_A")).toMatchObject([
      { fullyRefunded: true },
    ]);
    await expect(disputedPayment([]).provider.subscriptions("cus_A")).rejects.toThrow(
      "Dispute scope incomplete",
    );
    await expect(disputedPayment(["won"], true).provider.subscriptions("cus_A")).rejects.toThrow(
      "Dispute scope incomplete",
    );
  });
});
