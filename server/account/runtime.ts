...
export function accountRuntime() {
 ...
 const store: AccountStore = {
  ...
  async release(customer, token) { const { error } = await client.rpc("release_billing_lease", { p_customer: customer, p_token: token }); if (error) throw error; },
  async begin(owner, operation, customer, lease) { const { error } = await client.rpc("begin_account_deletion", { p_owner: owner, p_operation: operation, p_customer: customer, p_lease: lease }); if (error) throw error; },
  async assets(owner) {
   const rows: { bucket: string; path: string }[] = [];
   for (let offset = 0;; offset += 500) { const { data, error } = await client.from("user_assets").select("bucket_id,storage_path").eq("owner_id", owner).order("id").range(offset, offset + 499); if (error) throw error;
    rows.push(...(data ?? []).map(row => ({ bucket: row.bucket_id as string, path: row.storage_path as string }))); if (!data || data.length < 500) return rows;
   }
  },
  async removeAssets(assets) { for (let offset = 0; offset < assets.length; offset += 100) { const group = assets.slice(offset, offset + 100); if (group.some(asset => asset.bucket !== "private-assets")) throw new Error("Unknown private bucket"); const { error } = await client.storage.from("private-assets").remove(group.map(asset => asset.path)); if (error) throw error; } },
  async deleteUser(owner) { const { error } = await client.auth.admin.deleteUser(owner, false); if (error) throw error; },
 };
 const billing: AccountBilling = { async ensureNoRecurringCharges(customer) {
  const key = process.env.STRIPE_SECRET_KEY; if (!key || !/^sk_(test|live)_/.test(key)) throw new Error("Billing verification unavailable");
  if (key.startsWith("sk_live_") && (process.env.VERCEL_ENV !== "production" || process.env.STRIPE_ALLOW_LIVE !== "true")) throw new Error("Live billing not enabled");
  ...
  return !subscriptions.data.some(item => !["canceled", "incomplete_expired"].includes(item.status)) && sessions.data.length === 0;
 } };
 return createAccountHandlers({ store, billing, origin: origin.origin });
}
export async function accountDeleteEndpoint(request: Request) { try { return await accountRuntime().delete(request); } catch { return Response.json({ message: "Account deletion is not available in this environment yet." }, { status: 503, headers: { "Cache-Control": "no-store" } }); } }
