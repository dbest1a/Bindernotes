...
const metric = { release: "development", correlation: crypto.randomUUID(), operation: "content_save", result: "failed", durationMs: 10, retryCount: 1, payloadBytes: 200 };
const request = (body: unknown, token = "valid") => new Request("https://app.test/api/telemetry", { method: "POST", headers: { Authorization: `Bearer ${token}` }, body: JSON.stringify(body) });
describe("metrics ingestion", () => {
  it("authenticates and logs only strictly validated fields", async () => {
    const log = vi.fn(); const handler = telemetryHandler({ authenticate: async (token) => token === "valid", log });
  ...
  it("bounds actual request size regardless of a claimed content-length", async () => {
    const log = vi.fn(); const handler = telemetryHandler({ authenticate: async () => true, log });
