...
async function verifiedObject(response: Response, asset: PrivateAsset) {
  if (!response.ok || !response.body)
    throw new AssetError(409, "The upload is incomplete. Resume it before finishing.");
  ...
  try {
    for (;;) {
      ...
      if (size > asset.size_bytes) {
        await reader.cancel();
        throw new AssetError(422, "The uploaded file has an unexpected size.");
      }
      if (prefix.length < 16)
        prefix = new Uint8Array([...prefix, ...next.value.slice(0, 16 - prefix.length)]);
      hash.update(next.value);
    }
  } finally {
    reader.releaseLock();
  }
  const mime = detectPrivateAssetMime(prefix);
  const digest = hash.digest("hex");
  if (size !== asset.size_bytes || digest !== asset.sha256 || mime !== asset.mime_type)
    throw new AssetError(422, "The uploaded file did not pass verification. Remove it and try again.");
...
export function createAssetHandlers(deps: { store: AssetStore; origin: string; cleanupSecret?: string }) {
  ...
  async function owned(request: Request) {
    ...
    if (request.headers.get("origin") !== deps.origin)
      throw new AssetError(403, "Return to BinderNotes to manage private files.");
    ...
    try {
      ...
      throw new AssetError(401, "Sign in again to manage private files.");
    }
    const reader = request.body?.getReader();
    if (!reader) throw new AssetError(400, "Choose a file.");
    ...
    try {
      for (;;) {
        ...
        if (bytes > 1024) {
          await reader.cancel();
          throw new AssetError(413, "Request is too large.");
        }
    ...
    try {
      ...
      throw new AssetError(400, "Choose a valid file.");
    }
  ...
  const safe = (handler: (request: Request) => Promise<Response>) => async (request: Request) => {
    try {
      ...
      return error instanceof AssetError
        ...
        : json({ message: "File service is temporarily unavailable. Your upload can be retried." }, 503);
    }
  };
  async function remove(asset: PrivateAsset, alreadyClaimed = false) {
    if (!alreadyClaimed) await deps.store.markDeleting(asset);
    await deps.store.removeObject(asset);
    await deps.store.finishDeletion(asset);
  }
  return {
    complete: safe(async (request) => {
      ...
      if (asset.status === "ready" || asset.status === "staged")
        return json({ id: asset.id, status: asset.status });
    ...
    cleanup: safe(async (request) => {
      if (
        request.method !== "POST" ||
        !deps.cleanupSecret ||
        request.headers.get("authorization") !== `Bearer ${deps.cleanupSecret}`
      )
        throw new AssetError(403, "Cleanup is restricted.");
      const assets = await deps.store.stale();
      let removed = 0;
      for (const asset of assets) {
        if (await deps.store.claimStale(asset)) {
