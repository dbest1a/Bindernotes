...
export function assetRuntime() {
  if (
    ...
    throw new Error("Private files are not configured");
  ...
  const store: AssetStore = {
    ...
    async asset(id, owner) {
      ...
      return data ? privateAssetSchema.parse(data) : null;
    },
    async object(asset) {
      ...
      if (error) throw error;
      return fetch(data.signedUrl, { redirect: "error" });
    },
    async complete(asset, digest, size, mime) {
      const { error } = await client.rpc("complete_user_asset", {
        p_id: asset.id,
    ...
    async claimStale(asset) {
      const { data, error } = await client
        .from("user_assets")
        .update({ status: "deleting", updated_at: new Date().toISOString() })
        .eq("id", asset.id)
        .eq("owner_id", asset.owner_id)
        .in("status", ["pending", "staged", "deleting"])
    ...
    async removeObject(asset) {
      const { error } = await client.storage.from(asset.bucket_id).remove([asset.storage_path]);
      if (error) throw error;
    },
    async finishDeletion(asset) {
      const { error } = await client.rpc("finish_user_asset_deletion", {
        p_id: asset.id,
        p_owner: asset.owner_id,
      });
      if (error) throw error;
  ...
  return createAssetHandlers({
    store,
    origin: origin.origin,
    cleanupSecret: process.env.ASSET_CLEANUP_SECRET,
  });
}
export function assetEndpoint(kind: "complete" | "remove" | "cleanup") {
  return async (request: Request) => {
    try {
      return await assetRuntime()[kind](request);
    } catch {
      return Response.json(
        { message: "Private file uploads are not available in this environment yet." },
